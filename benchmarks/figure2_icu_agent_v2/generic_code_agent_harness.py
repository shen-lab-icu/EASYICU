...
ALLOWED_LANGUAGES = frozenset({"python", "r", "shell"})
...
class DockerRunnerBackend:
    """Adapt the existing isolated Python runner to Python, R, and shell."""
    ...
    def _interpreter_wrapper(language: str, code: str) -> str:
        command = {"r": "Rscript", "shell": "bash"}[language]
        suffix = {"r": ".R", "shell": ".sh"}[language]
        return (
            "from pathlib import Path\n"
            "import subprocess\n"
            "import sys\n\n"
            f"script_path = Path('/tmp/generic-agent-action{suffix}')\n"
            f"script_path.write_text({code!r}, encoding='utf-8')\n"
            f"completed = subprocess.run([{command!r}, str(script_path)], check=False)\n"
        ...
        messages.append(
            LLMMessage(
                ...
                content=(
                    ...
                    "Use {\"action\":\"execute\",\"language\":\"python|r|shell\","
        ...
        receipt = {
            **snapshot,
            ...
            "mandatory_artifact_presence": {
                name: True for name in CANONICAL_FILES[:6]
            },
            "model_turns": model_turns,
            "tool_calls": tool_calls,
