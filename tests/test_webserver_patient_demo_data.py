...
def test_patient_seeded_demo_uses_clinical_table_shape() -> None:
    ...
    assert "const DEMO_CHART_HOURS = [0.2, 1, 2, 3.4, 6, 8, 12, 18, 24, 30, 36, 48];" in demo_js
    ...
    assert "if (timeIndexed) out.charttime = context.charttime;" in viz_js
    assert "out[feature] = demoTableValue(feature, context.valueSeed + featureIdx + moduleIdx);" in viz_js
