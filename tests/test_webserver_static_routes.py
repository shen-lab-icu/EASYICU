...
def test_native_assistant_labels_disambiguate_page_guide_guided_copilot_and_agent_guide() -> (
    ...
    assert "js/app.js?v=20260729-spine4" in index_html
...
def test_native_guided_and_page_guide_messages_are_bilingual() -> None:
    ...
    assert "js/screens-guided.js?v=20260729-spine4" in index_html
    assert "js/copilot-dock.js?v=20260712-ux-fixes" in index_html
...
def test_native_guided_copilot_runs_extraction_inline_and_answers_catalog_questions() -> (
    ...
    assert "css/guided.css?v=20260729-spine4" in index_html
    ...
    extract_js = _static_js("screens-guided-extract.js")
    assert "js/screens-guided-extract.js?v=20260729-spine4" in index_html
...
def test_native_agent_outputs_fail_closed_to_real_artifacts() -> None:
    ...
    assert "js/screens-agent.js?v=20260712-ux-fixes" in index_html
    assert "css/agent.css?v=20260729-spine4" in index_html
...
def test_native_agent_research_blocks_are_project_owned() -> None:
    ...
    assert ".ag-wf-guide" in agent_css
...
def test_panel_mount_runs_each_screen_afterrender() -> None:
    """A screen is render() AND afterRender(). Panels must call both.

    Mounting screens as panels moved them off the shell's render path, which
    calls afterRender for the route it is showing. The panel host called only
    render(), so all ten panel screens came up as dead shells — no fetch, no
    chart, no listener. Measured: Patient Review showed its "official public
    ICU demos" heading above an EMPTY grid while the API had two prepared
    sources ready; opening the demo went from 227 to 843 characters once
    afterRender ran, and the loaded workspace from nothing to four tabs
    (数据表 / 时间序列 / 患者概览 / 数据质量) with 14 charts.

    Every id in PANEL_ROUTES defines afterRender, so there is no such thing as
    a panel screen that does not need this.
    """
    one_js = _static_js("screens-one.js")
    app_js = _static_js("app.js")

    # The panel host calls it, and only after the node is in the document —
    # screens query the live tree, and a detached subtree finds nothing.
    assert "afterRender(host)" in one_js
    mount = one_js.split("function mountPanel", 1)[1]
    assert mount.index("aside.appendChild(host)") < mount.index("afterRender(host)")
    # One screen's setup must not take the conversation down with it.
    assert "console.warn('[EasyICU] panel afterRender failed:'" in one_js

    # The conversation keeps its own call; the two paths are not merged.
    assert "if (chat.afterRender) chat.afterRender(app);" in app_js
...
def test_native_guided_publishes_the_stated_question_downstream() -> None:
    ...
    The shared snapshot every downstream module consumes took its question
    from the AGENT card's box, which is seeded with a synthesised English
    objective — so a study stated as 「脓毒症患者前 48 小时乳酸能不能预测院内
    死亡」 arrived everywhere as "Evaluate the active ICU cohort with an
    evidence-bound local preflight." and outcome:"" — while the window from
    the same sentence came through correctly, because the window published
    unconditionally and the outcome was gated behind a flag only the Design
    step set.
    """
    guided_js = _static_js("screens-guided.js")

    assert "function guidedStatedQuestion" in guided_js
    # The reader's own words outrank the synthesised objective, in both the
    # snapshot and the agent card's seed.
    assert "question: guidedStatedQuestion()" in guided_js
    assert "const stated = guidedStatedQuestion();" in guided_js

    # A set value publishes, whichever step set it — same as the window always did.
    assert "outcome: guidedDesignOutcome()," in guided_js
    assert "guidedDesign.collected ? guidedDesignOutcome() : ''" not in guided_js

    # `collected` (has values) and `reviewed` (walked the Design step) are
    # separate, because only the second is a confirmation the reader made.
    assert "collected: false, reviewed: false" in guided_js
    assert "guidedDesign.reviewed = true;" in guided_js
    assert "configured: !!(guidedDesign && guidedDesign.reviewed)" in guided_js

    # Every goal that can carry the sentence does. run_agent used to drop it.
    assert "startGuidedExtractionFlow(echo, seed)" in guided_js
    assert "if (seed) { guidedExtract.question = seed; commitGuidedQuestion(); }" in guided_js

    # The study folder is named after the research, not the button.
    assert "const stated = String(guidedFrontdoorSeedText || '').trim();" in guided_js
...
def test_native_guided_local_rail_shows_only_real_local_context() -> None:
    ...
    assert "screens-guided.js?v=20260729-spine4" in index_html
    assert "guided.css?v=20260729-spine4" in index_html
    assert "guided-rail.css?v=20260729-spine4" in index_html
