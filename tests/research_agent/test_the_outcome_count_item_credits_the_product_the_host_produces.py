...
def test_the_runs_that_reported_it_open_did_hold_that_table() -> None:
    ...
    for checklist_path in sorted(
        ...
        if str(item.get("status")) != "open":
            unexplained.append((checklist_path.parent.name, item.get("status")))

    if not covered:
        pytest.skip(
            f"no recorded run holds the owner's table ({without_the_owner} predate it)"
        )
    assert not unexplained, (
        "a recorded run holds the owner's table yet did not report the "
        f"outcome-count item open, so this is not the gap it looks like: {unexplained[:5]}"
    )
