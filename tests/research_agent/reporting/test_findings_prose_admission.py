...
@pytest.mark.parametrize(
    "methods,results", [("Methods:", "Results:"), ("方法：", "结果：")]
)
def test_structural_abstract_labels_preserve_the_findings_boundary(
    prefix, separator, methods, results
) -> None:
    method = "We fitted the prespecified regression model."
    finding = "The treatment spared the kidneys."
    result = filter_evidence_bound_scaffold(
        f"## Abstract\n{prefix}**{methods}**{separator}{method}\n"
        f"{prefix}**{results}**{separator}{finding}\n",
        resolve_claim=lambda _: None,
    )
    assert method in result.scaffold
    assert finding not in result.scaffold
    assert finding in result.unsupported_scientific_claim_sentences


@pytest.mark.parametrize("prefix", ["**Results:** ", "> - **Results:** ", "**结果：** "])
def test_structural_findings_label_cannot_release_unregistered_fragment(prefix) -> None:
    finding = "The treatment spared the kidneys."
    result = filter_evidence_bound_scaffold(prefix + finding, resolve_claim=lambda _: None)
    assert finding not in result.scaffold
    assert result.unsupported_scientific_claim_sentences == (finding,)
