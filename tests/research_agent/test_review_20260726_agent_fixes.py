...
# ---------------------------------------------------------------------------
# P1-6 / P1-7 — MCP admission control and anonymous loopback authority
# ---------------------------------------------------------------------------


def test_p1_6_unknown_session_is_rejected_before_the_tool_runs(monkeypatch):
    import http.client
    import http.server
    import threading
    ...
    monkeypatch.setitem(mcp.TOOLS, "research_agent.list_skills", _tracked)
        ...
        body = json.dumps(
            {
                ...
                "id": 1,
                "method": "tools/call",
                "params": {"name": "research_agent.list_skills", "arguments": {}},
            }
        ).encode("utf-8")
    ...
    assert status == 404
    # The whole point: the extraction never ran for a reply nobody can receive.
    assert executed == []


def test_p1_7_anonymous_loopback_cannot_start_a_run_or_write(monkeypatch):
    import http.client
    import http.server
    import threading
    ...
    monkeypatch.setattr(mcp, "ResearchAgentPipeline", _NeverRuns)
    ...
    def _call(name, arguments):
        ...
        connection.request(
            "POST", "/jsonrpc", body=body, headers={"Content-Type": "application/json"}
        )
    ...
    try:
        run = _call("research_agent.run", {"question": "q", "cohort_path": "c"})
        skills = _call("research_agent.list_skills", {})
    finally:
        server.shutdown()
        server.server_close()
        thread.join(timeout=2)

    assert "scope_not_granted" in json.dumps(run)
    assert started == []
    # Read-only metadata still works without a token.
    assert "error" not in json.dumps(skills["result"]["content"][0]["text"])[:40]
