...
from tests.support.robustness_sources import (
    write_structured_source_authority as _write_structured_source_authority,
)
...
def test_structured_preflight_replays_exact_primary_code_and_emits_spec_by_model_evidence(
    ...
    # Inheriting the primary's own fit restates the primary: it keeps the fit
    # as fitted but is no variant.  The primary step's separate complete-case
    # fit is one.
    assert missing_rows.loc["missing_complete_case", "estimability_status"] == "estimated"
    assert missing_rows.loc["missing_source_aware", "estimability_status"] == (
        "not_independent"
    )
    assert bool(missing_rows.loc["missing_source_aware", "converged"])
    assert not pd.isna(missing_rows.loc["missing_source_aware", "point_estimate"])
    outcome_rows = matrix[matrix["axis"] == "outcome"]
