...
from easyicu.research_agent.planning.progressive_compiler import (
    ...
    _compile_ordered_stratified_contract,
...
def test_primary_contrast_uses_declared_ordinal_domain_without_patient_rows() -> None:
    """A metadata-only plan may use an owner-declared clinical level set."""
    ...
    step_payload = deepcopy(_payload()["steps"][4])
    step_payload.update(
        {
            "raw_inputs": ["aki_stage_max", "outcome_flag"],
            ...
            "primary_contrast_level_index": 3,
        }
    )
    step = ProgressiveSkeletonStep.model_validate(step_payload)

    _terms, _covariates, levels, reference, contrast = _compile_model_terms(
...
def test_typed_ordinal_multi_outcome_context_exposes_optional_trend_action() -> None:
    context = ResearchContext(
        ...
        cohort=CohortDescriptor(
            ...
            n_stays=120,
            id_columns=["stay_id"],
            outcome_columns=["death", "los_days"],
        ),
        variables=[
            ConceptDescriptor(
                name="stage",
...
def _metadata_only_ordinal_multi_outcome_context() -> ResearchContext:
    return ResearchContext(
        research_question="Assess an ordered stage against mortality and length of stay.",
        cohort=CohortDescriptor(
            cohort_name="synthetic",
            database="synthetic",
            n_stays=0,
            id_columns=["stay_id"],
            outcome_columns=["los_days", "death"],
        ),
        variables=[
            ConceptDescriptor(
                name="stage",
...
def _ordinal_primary_step(*, coding: str) -> ProgressiveSkeletonStep:
    return ProgressiveSkeletonStep.model_validate(
        {
            "step_id": "primary_adjusted_model",
            "planned_analysis_role": "primary",
            "module_id": "adjusted_association",
            "objective": "Estimate the adjusted stage and mortality association.",
            "raw_inputs": ["stage", "death"],
            "primary_exposure": "stage",
            "outcome": "death",
            "outcome_type": "binary",
            "model_terms": [
                {
                    ...
                    "role": "exposure",
                    "coding": coding,
                    "reference_level_index": 0 if coding == "categorical" else None,
                }
            ],
            "event_level_index": 1,
            "primary_contrast_level_index": 3,
...
def test_ordered_trend_compiler_accepts_row_free_typed_metadata() -> None:
    ...
    parent = _ordinal_primary_step(coding="categorical")
    trend = ProgressiveSkeletonStep.model_validate(
        {
            "step_id": "ordinal_trend_secondary",
            "planned_analysis_role": "secondary",
            "module_id": "custom_analysis",
            "objective": "Test ordered gradients for mortality and length of stay.",
            "depends_on": [parent.step_id],
            "raw_inputs": ["stage", "death", "los_days"],
            "outputs": [
                {
                    "product_id": "table:ordinal_trend_results",
    ...
    skeleton = ProgressivePlanSkeleton(
        ...
        steps=[parent, trend],
        rationale="Keep categorical stage effects separate from the ordered trend test.",
    )

    assert _compile_ordered_stratified_contract(
        context=context,
        skeleton=skeleton,
        step=trend,
        step_index=1,
        variables={variable.name: variable for variable in context.variables},
        output_pairs=[("table:ordinal_trend_results", "custom")],
    )


def test_host_closes_ordered_trend_materialization_inputs_from_typed_action() -> None:
    context = _metadata_only_ordinal_multi_outcome_context()
    parent = _ordinal_primary_step(coding="categorical")
    outline_step = ProgressiveOutlineStep(
        ...
        objective="Test ordered gradients for mortality and length of stay.",
        depends_on=[parent.step_id],
        variable_names=["stage", "death", "los_days"],
        scientific_action_id="association.ordinal_trend",
    )
    materialization = ProgressiveStepMaterialization(
        ...
        step=ProgressiveSkeletonStep(
            ...
            depends_on=[parent.step_id],
            raw_inputs=["stage", "death", "age", "sex"],
            outputs=[
                {
                    "product_id": "table:ordinal_trend_results",
                    "semantic_role": "custom",
            ...
            scientific_action_id="association.ordinal_trend",
            custom_method="prespecified_ordinal_trend_association",
        ),
    )

    normalized = normalize_progressive_action_contract(
        materialization,
        context=context,
        outline_step=outline_step,
        available_product_refs=[
            (parent.step_id, "table:adjusted_association_estimates")
        ],
    )

    assert normalized.step.raw_inputs == ["stage", "death", "los_days"]
    assert [
        (reference.producer_step_id, reference.product_id)
        for reference in normalized.step.product_inputs
    ] == [(parent.step_id, "table:adjusted_association_estimates")]
    skeleton = ProgressivePlanSkeleton(
        analysis_type="association_study",
        cohort=ProgressiveCohortIntent(
...
def test_outline_requires_direct_comparator_on_primary_step() -> None:
    ...
    for step in outline_payload["steps"]:
        if step["planned_analysis_role"] in {"primary", "secondary", "sensitivity"}:
            step["literature_citation_keys"] = ["direct_comparator"]
    ProgressivePlannerAgent._validate_outline_authority(
...
def test_descriptive_prompt_binds_registered_table_one_action() -> None:
    ...
    assert (
        "A table_one step must bind descriptive.table_one when that exact "
        "action appears" in prompt
    )


def test_descriptive_outline_requires_registered_table_one_action() -> None:
    payload = _payload()
    payload["analysis_type"] = "descriptive_epidemiology"
    payload["robustness_intents"] = []
    payload["steps"] = payload["steps"][:4]
    payload["steps"][1]["planned_analysis_role"] = "secondary"
    ...
    with pytest.raises(ProgressivePlanCompileError) as caught:
        ProgressivePlannerAgent._validate_outline_authority(
            ...
            closed_domain_variables=["exposure_flag", "outcome_flag", "sex_code"],
        )
    ...
    outline_payload["steps"][1]["scientific_action_id"] = "descriptive.table_one"
    ProgressivePlannerAgent._validate_outline_authority(
        ProgressivePlanOutline.model_validate(outline_payload),
        analysis_types=["descriptive_epidemiology"],
        variable_names=[variable.name for variable in _context().variables],
...
def test_metadata_only_descriptive_fallback_survives_family_module_gate() -> None:
    payload = _payload()
    payload["analysis_type"] = "descriptive_epidemiology"
    payload["robustness_intents"] = []
    payload["steps"] = payload["steps"][:4]
    ...
    assert baseline.method == "descriptive_cohort_summary"
    assert baseline.expected_outputs == ["table:cohort_summary"]
...
def test_article_contract_credits_host_compiled_data_quality_audit_product() -> None:
    ...
    covered = roles_covered_by_plan(
        plan,
        build_article_analysis_contract(_context(), analysis_type=plan.analysis_type),
    )
...
def test_article_contract_credits_typed_measurement_audit_with_precise_products() -> None:
    payload = _payload()
    measurement_payload = payload["steps"][3]
    measurement_payload["outputs"] = [
        {
            "product_id": "table:data_quality_measurement_process",
            "semantic_role": "measurement_process",
        },
        {
            "product_id": "table:data_quality_measurement_missingness",
            "semantic_role": "measurement_missingness",
        },
    ]
    plan, _receipt = compile_progressive_plan(
    ...
    assert measurement.measurement_audit_spec is not None
...
def test_host_distribution_binds_sealed_repeated_unit_method_source() -> None:
    outline_payload = _outline_payload()
    outline_payload["steps"][2]["literature_citation_keys"] = ["strobe_2007"]
    outline = ProgressivePlanOutline.model_validate(outline_payload)
    foundation = ProgressiveFoundationMaterialization.model_validate(
        _foundation_payload()
    ).foundation
    ...
    bindings = {
        ...
        for binding in distribution.step.literature_bindings
    }
    assert "dependence" in bindings["strobe_2007"]
...
def test_host_normalizes_proven_identity_without_dropping_planner_bindings() -> None:
    ...
    assert (
        normalized.step.literature_bindings == materialization.step.literature_bindings
    )
...
def test_host_reuses_selected_design_decision_for_dynamic_figure_binding() -> None:
    ...
    assert (
        "Show cohort accounting"
        in materialization.step.literature_bindings[0].application
    )
...
def test_checkpoint_resume_reuses_host_materialized_null_schema_authority(
    ...
    assert resume_checkpoint.prompt_metrics["step_materialization_schema_sha256"][
        :3
    ] == [None, None, None]
...
def test_outline_parser_canonicalizes_only_bijective_internal_step_coordinates() -> (
    None
):
    payload = _outline_payload()
...
def test_materialization_prompt_projects_exact_method_source_scope() -> None:
    ...
    assert (
        '"citation_key":"record_2015","allowed_design_elements":["reporting"]' in prompt
    )
...
def test_association_custom_sensitivity_must_follow_primary_adjusted_model() -> None:
    ...
    primary = next(
        step for step in payload["steps"] if step["planned_analysis_role"] == "primary"
    )
...
def test_scientific_measurement_audit_seals_missing_data_method_source() -> None:
    payload = _outline_payload()
    payload["steps"] = [payload["steps"][0], payload["steps"][3]]
    payload["steps"][1]["planned_analysis_role"] = "primary"
    payload["steps"][1]["literature_citation_keys"] = ["strobe_2007"]
    ...
    bound = _bind_required_outline_method_sources(
        ...
        context_required_method_layers=("dependence", "reporting_standard"),
        continuous_domain_variables=(),
    )

    measurement = bound.steps[1]
...
def test_host_binds_screened_direct_comparator_to_primary_without_a_retry() -> None:
    outline = ProgressivePlanOutline.model_validate(_outline_payload())
    primary = next(
        step for step in outline.steps if step.planned_analysis_role == "primary"
    )
    ...
    assert bound_primary.literature_citation_keys == ["direct_lactate_mortality"]
...
def test_foundation_parser_binds_host_outline_digest_without_rewriting_choices() -> (
    None
):
    payload = _foundation_payload()
    payload["outline_sha256"] = "f" * 64
    payload["foundation"]["cohort"]["name"] = "model_selected_cohort"
    ...
    assert parsed.outline_sha256 == "a" * 64
    assert parsed.foundation.cohort.name == "model_selected_cohort"
...
def test_visualization_schema_hides_products_outside_direct_dependencies() -> None:
    ...
    product_refs = json.loads(request.schema_json)["$defs"]["ProgressiveProductRef"][
        "anyOf"
    ]

    assert len(product_refs) == 1
    assert product_refs[0]["properties"]["producer_step_id"]["const"] == ("05_primary")
...
def test_progressive_covariate_requires_agent_authored_clinical_rationale() -> None:
    payload = deepcopy(_payload()["steps"][4])
    payload["model_terms"][1].pop("clinical_rationale")

    with pytest.raises(ValueError, match="Agent-authored clinical_rationale"):
        ProgressiveSkeletonStep.model_validate(payload)


def test_adjusted_prompt_projects_exact_model_term_authority() -> None:
    context = _context().model_copy(
        update={
            "variables": [
                *_context().variables,
    ...
    outline = ProgressivePlanOutline(
        analysis_type="association_study",
        cohort_objective="Use the sealed cohort.",
        steps=[outline_step],
        rationale="Use the registered adjusted association module.",
    )
    ...
    prompt = ProgressivePlannerAgent._materialization_prompt(
        ...
        outline_step=outline_step,
        outline_step_sha256=canonical_sha256(outline_step.model_dump(mode="json")),
        variables=outline_step.variable_names,
        action_rows=[],
        allowed_literature_citation_keys=[],
        know_how_context="",
        planning_contract_context="",
    ...
    assert "Host model-term planning authority (binding)" in prompt
    assert '"name":"age_years","semantic_role":"demographic"' in prompt
    assert '"name":"sex_code","semantic_role":"demographic"' in prompt
    assert '"allowed_codings":["binary"]' in prompt
    assert (
        '"name":"ventilation_status","reason":"planner_baseline_authority_missing"'
        in prompt
    )
    assert (
        '"name":"outcome_flag"'
        not in prompt.split("Host model-term planning authority (binding):", 1)[1]
    )
...
def test_compiler_materializes_host_owned_contracts_and_exact_wires() -> None:
    ...
    assert requirement.covariate_rationales == {
        "age_years": (
            "Age can confound the association because it precedes exposure "
            "ascertainment and relates to outcome risk."
        ),
        "sex_code": (
            "Sex can confound the association because it is fixed before exposure "
            "ascertainment and relates to risk."
        ),
    }
...
def test_compiler_uses_declared_integer_range_for_metadata_only_ordinal_score() -> None:
    ...
    context = _context().model_copy(
        update={
            ...
            "user_preferences": UserPreferences(
                covariates=["age_years", "sex_code", "severity_total"],
                covariate_selection="exact",
                covariate_rationales={
                    "age_years": "Prespecified baseline age confounder.",
                    "sex_code": "Prespecified baseline sex confounder.",
                    "severity_total": "Prespecified baseline severity confounder.",
                },
                covariate_temporal_roles={
                    "age_years": "baseline_static",
                    "sex_code": "baseline_static",
                    "severity_total": "at_or_before_time_zero",
                },
            ),
        }
    )
...
def test_outline_binds_optional_ordered_trend_only_when_selected() -> None:
    ...
    payload["steps"].append(
        {
            "step_id": "ordered_trend_audit",
            "planned_analysis_role": "secondary",
            "module_id": "custom_analysis",
            "objective": (
                "Audit ordered outcome gradients on the primary analysis lineage."
            ),
            "depends_on": [primary["step_id"]],
            "variable_names": list(requirement),
            "literature_citation_keys": [],
            "scientific_action_id": "association.ordinal_trend",
        }
    )
...
def test_outline_rejects_table_one_without_closed_group_domain() -> None:
    ...
    assert (
        caught.value.reason_code == "progressive_outline_table_one_domain_unavailable"
    )
...
def test_outline_requires_functional_form_sensitivity_for_exact_continuous_covariate() -> None:
    payload = _outline_payload()
    selected = next(
        candidate
        for candidate in payload["design_selection"]["candidates"]
    ...
    selected["required_variables"].append("age_years")
    ...
    payload["steps"].append(
        {
            ...
            "planned_analysis_role": "sensitivity",
            "module_id": "custom_analysis",
            "objective": "Check the prespecified continuous covariate functional form.",
            "depends_on": ["05_primary"],
...
def test_outline_prompt_requires_functional_form_step_before_plan_review() -> None:
    context = _context().model_copy(
        update={
            "user_preferences": UserPreferences(
                covariates=["AGE"],
...
def test_retrieved_data_cards_use_concept_declared_domain_without_claiming_observation() -> (
    None
):
    context = _context().model_copy(
        update={
...
def test_outline_prompt_projects_exact_case_neutral_json_shape() -> None:
    ...
    assert (
        '"literature_citation_keys":["<sealed method key for every '
        'primary/secondary/sensitivity step>"]' in prompt
    )
    assert "only auxiliary steps may use an empty array" in prompt
...
def test_foundation_contract_projects_required_reader_label_keys() -> None:
    contract = _foundation_shape_contract(
        outline_sha256="a" * 64,
        host_cohort=None,
...
def test_descriptive_figure_is_owned_by_post_extraction_renderer() -> None:
    context = _context().model_copy(
        update={
            "user_preferences": UserPreferences(
                inferred_analysis_family="descriptive_epidemiology",
    ...
    assert _requires_visualization_step(context) is False
    assert "Host-resolved presentation obligation" not in prompt
    assert "visualization" not in progressive_module_ids_for_analysis_types(
...
def test_functional_form_sensitivity_requires_an_exact_executable_method() -> None:
    unsupported = _payload()
    step = unsupported["steps"][5]
    step["step_id"] = "06_functional_form_sensitivity"
    step["custom_method"] = "continuous_functional_form_check"
    step["sensitivity_spec_ids"] = ["age_restricted_cubic_spline_vs_linear"]

    with pytest.raises(ProgressivePlanCompileError) as caught:
        compile_progressive_plan(
            skeleton=ProgressivePlanSkeleton.model_validate(unsupported),
            context=_context(),
        )

    assert caught.value.reason_code == "progressive_functional_form_method_unsupported"
    assert caught.value.path == "custom_method"
    ...
    plan, _receipt = compile_progressive_plan(
        skeleton=ProgressivePlanSkeleton.model_validate(supported),
        context=_context(),
    )

    compiled = next(
        item for item in plan.steps if item.step_id == "06_functional_form_sensitivity"
    )
    assert compiled.method == "restricted_cubic_spline_sensitivity"
...
def test_compiler_uses_concept_declared_levels_for_metadata_only_model_term() -> None:
    ...
    term = next(item for item in requirement.model_terms if item.name == "sex_declared")
...
def test_compiler_rejects_a_different_outcome_as_baseline_covariate() -> None:
    context = _context().model_copy(
        update={
            "variables": [
                *_context().variables,
    ...
    payload = _payload()
    primary = payload["steps"][4]
    primary["raw_inputs"].append("length_of_stay")
    primary["model_terms"].append(
        {
            "name": "length_of_stay",
            "role": "covariate",
...
def test_compiler_rejects_dynamic_covariate_without_time_zero_authority() -> None:
    context = _context().model_copy(
        update={
            "variables": [
                *_context().variables,
    ...
    payload = _payload()
    primary = payload["steps"][4]
    primary["raw_inputs"].append("ventilation_status")
    primary["model_terms"].append(
        {
            "name": "ventilation_status",
            "role": "covariate",
    ...
    assert caught.value.path == "model_terms[3]"
    assert "availability in the source catalog" in str(caught.value)
...
def test_compiler_binds_missing_data_card_for_scientific_audit_without_replay() -> None:
    payload = _payload()
    payload["analysis_type"] = "descriptive_epidemiology"
    payload["robustness_intents"] = []
    payload["steps"] = [payload["steps"][0], payload["steps"][3]]
    ...
    measurement = next(step for step in plan.steps if step.step_id == "04_measurement")
    assert measurement.literature_citation_keys == ["sterne_missing_data_2009"]
    assert measurement.literature_design_bindings[0].design_elements == [
        "missing_data",
        "robustness",
    ]
...
def test_outline_uses_fourth_attempt_after_three_distinct_contract_repairs() -> None:
    ...
    missing_figure["steps"] = [
        step for step in missing_figure["steps"] if step["module_id"] != "visualization"
    ]
    ...
    assert "progressive_outline_action_unavailable" in (llm.calls[2][0][-1].content)
...
def test_agent_repairs_only_the_current_materialization() -> None:
    ...
    assert (
        len(
            agent.last_result.facts.prompt_metrics["step_materialization_schema_sha256"]
        )
        == 7
    )
    assert (
...
def test_agent_repairs_two_independent_current_step_findings() -> None:
    materializations = _materialization_payloads()
    unknown_covariate = deepcopy(materializations[4])
    unknown_covariate["step"]["model_terms"][1]["name"] = "invented_covariate"
    outcome_covariate = deepcopy(materializations[4])
    outcome_covariate["step"]["model_terms"].append(
        {
            "name": "outcome_flag",
            "role": "covariate",
            "coding": "binary",
    ...
    assert "progressive_unknown_variable" in llm.calls[7][0][-1].content
    assert "progressive_model_outcome_is_covariate" in llm.calls[8][0][-1].content
    assert agent.last_result.facts.prompt_metrics["compile_revision_count"] == 2
    assert agent.last_result.facts.prompt_metrics["step_materialization_count"] == 7
    assert agent.last_result.facts.prompt_metrics["full_revision_count"] == 0
...
def test_agent_repairs_primary_model_to_the_exact_adjustment_roster() -> None:
    ...
    selected["required_variables"].extend(["age_years", "sex_code"])
    foundation = _foundation_payload(payload)
    ...
    repair_prompts = [
        ...
        if "HOST COMPILER OBSERVATION" in messages[-1].content
    ]
    assert any(
        "progressive_adjustment_authority_mismatch" in prompt
        for prompt in repair_prompts
    )
    assert any(
        '"required_covariates":["age_years","sex_code"]' in prompt
        for prompt in repair_prompts
    )
    assert agent.last_result.facts.prompt_metrics["compile_revision_count"] == 1
...
def test_agent_stops_after_two_host_compile_repairs_and_keeps_attempts() -> None:
    materializations = _materialization_payloads()
    invalid_distribution = json.loads(json.dumps(materializations[2]))
    invalid_distribution["step"]["comparison_exposure_level_index"] = 0
    responses = [
    ...
    assert failure_facts.prompt_metrics["compile_revision_count"] == 2
    assert len(failure_facts.compile_failure_attempts) == 3
