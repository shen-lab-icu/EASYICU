...
def test_nonfinite_guard_repair_is_centrally_authorized_as_structural() -> None:
    metadata = repair_metadata_for("conditional_nonfinite_fail_closed_guard_v1")

    assert metadata.repair_class is RepairClass.STRUCTURAL
    assert automatic_repair_allowed(metadata.repair_id) is True
