...
def test_composite_audits_stay_when_the_main_figure_needs_their_roles():
    """An omission may not remove coverage the article-figure gate requires.

    Measured 2026-09-12 on a sealed E2 plan: this omission ran first and left the
    composite with its two curve panels, after which the article-figure gate
    reported the promoted main figure for covering 2 < 3 required visual roles,
    with no panel profile available that could reach three.  The two planning
    passes simply cancelled each other out.  When measurement quality is part of
    the scientific problem, the audit panel *is* the third role, so the omission
    has to stand down rather than ship a plan that can only fail the gate.
    """
    ...
    def _context(notes: str) -> ResearchContext:
        return ResearchContext(
            research_question="Is the biomarker associated with mortality?",
    ...
    headline_quality = _context(
        "Quantifying the measurement process and missingness is part of this study."
    )
    routine_quality = _context("")
    required_main, minimum = primary_publication_role_requirement(
        build_article_figure_strategy(headline_quality)
    )
    assert "data_quality" in required_main
    assert minimum == 3
    assert "data_quality" not in primary_publication_role_requirement(
        build_article_figure_strategy(routine_quality)
    )[0]

    def _plan_with_composite() -> AnalysisPlan:
        plan = _plan_with_typed_data_quality_sources()
        plan, _ = ensure_data_quality_figure_step(
            plan=plan, context=_Ctx(plan.research_question)
        )
        ...
        plan.steps.extend([
            ...
            AnalysisStep(
                ...
                intent="Show sensitivity.",
                method="visualization",
                inputs=["table:robustness_summary", "table:robustness_matrix"],
                expected_outputs=["figure:robust"],
                input_consumption_contracts=[
                    {"input_key": key, "mode": "all_rows"}
                    for key in ("table:robustness_summary", "table:robustness_matrix")
                ],
            ),
            AnalysisStep(
                step_id="composite",
                ...
                expected_outputs=["figure:curves"],
                input_consumption_contracts=[
                    {"input_key": key, "mode": "all_rows"} for key in sources
                ],
                figure_panels=[
                    panel.bind(figure_output="figure:curves")
    ...
    stripped, findings = omit_redundant_composite_audits(
        plan=_plan_with_composite(), context=routine_quality
    )
    composite = next(step for step in stripped.steps if step.step_id == "composite")
    assert [panel.article_role for panel in composite.figure_panels] == [
        "primary_estimand",
        "descriptive_result",
    ]
    assert findings and findings[0].detail["reason"] == (
        "composite_audits_have_dedicated_displays"
    )

    kept, kept_findings = omit_redundant_composite_audits(
        plan=_plan_with_composite(), context=headline_quality
    )
    kept_composite = next(step for step in kept.steps if step.step_id == "composite")
    assert list(kept_composite.inputs) == [
        "table:biomarker_rcs_curve",
        "table:biomarker_adjusted_absolute_risk",
        "table:robustness_summary",
        "table:measurement_process_audit",
    ]
    assert [panel.panel_id for panel in kept_composite.figure_panels] == [
        "association_curve",
        "absolute_risk_curve",
        "robustness_summary",
        "measurement_process",
    ]
    assert [finding.detail["reason"] for finding in kept_findings] == [
        "composite_audits_kept_for_main_figure_coverage"
    ]
    assert kept_findings[0].detail["duplicated_display_sources"] == [
        "table:measurement_process_audit",
        "table:robustness_summary",
    ]
    # The retained profile is the one the gate can actually pass, and the
    # stripped profile is the one it cannot -- asserted against the same
    # predicate both planning passes now read, so the three cannot drift.
    from easyicu.research_agent.planning.figure_strategy import (
        primary_publication_figure_is_article_grade,
    )

    strategy = build_article_figure_strategy(headline_quality)
    full = [
        (str(panel.article_role), str(panel.chart_type))
        for panel in landmark_association_composite_panels(
            list(kept_composite.inputs)
        )
    ]
    pair = [
        (str(panel.article_role), str(panel.chart_type))
    ...
    assert primary_publication_figure_is_article_grade(strategy, full) is True
    assert primary_publication_figure_is_article_grade(strategy, pair) is False
