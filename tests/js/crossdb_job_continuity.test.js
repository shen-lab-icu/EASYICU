...
function harness(options) {
  ...
  return { calls, context, EventSource: FakeEventSource, localStorage, snapshotCalls: () => snapshotCalls };
}
...
(async () => {
  ...
  running.EventSource.instances[0].message({ type: 'end', status: 'done', result });
  assert.deepEqual(running.calls.at(-1).slice(0, 3), ['terminal', META.job_id, 'done']);
  assert.equal(JSON.stringify(running.calls.at(-1)[3]), JSON.stringify(result));
  ...
  process.stdout.write(JSON.stringify({ restored: true, terminal_statuses: 3, missing_cleared: true, root_guard: true }));
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
