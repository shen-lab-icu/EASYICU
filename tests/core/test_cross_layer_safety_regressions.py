...
def test_eicu_offsets_outside_the_icu_episode_are_quarantined():
    ...
    2026-09-19 v6: eICU lab carried corrupt offsets (-52,578,464 min for
    stays 2385766/1699503 -> -876,308 h after /60) into staging; their
    hourly SOFA grids (~1.75M rows/stay) tripped the sep3 replay bound
    (DerivationContextError). The eICU path now enforces the same
    [-24 h, los_icu + 24 h] window the native-v2 publisher enforces
    (MIMIC bcb5032f / AUMC precedent). Inputs are minutes, outputs hours.
    """
    ...
    frame = pd.DataFrame(
        {
            # minutes: -52,578,464 (corrupt) / -25h / -24h / 0 / 72h / 73h / unknown-stay
