...
EVENT_DEFAULT_FALSE = (
    "rrt", "mech_vent", "ecmo", "death", "circ_event", "circ_failure",
    "aki", "sep3_sofa2", "susp_inf",
)
...
def build_full_wide_cohort(export_dir: Path, output_path: Path) -> dict:
    ...
    con.execute(
        f"COPY wide TO '{output_path.as_posix()}' (FORMAT parquet)"
    )
    n = con.execute("SELECT count(*) FROM wide").fetchone()[0]
    nonmiss = {}
    for c in concepts:
        frac = con.execute(
            f'SELECT count("{c}")*1.0/count(*) FROM wide'
        ).fetchone()[0]
...
def fetch_concept_scoped_articles(retmax: int, *, term: str | None = None) -> list[dict]:
...
def main() -> None:
    ...
    ap.add_argument(
        "--scope",
        choices=["concept", "generic", "prognostic", "labmarker", "diverse", "routine2outcome"],
        default="prognostic",
    ...
    print(
        f"[2/5] fetching real PubMed abstracts (scope={args.scope}) ...", flush=True
    )
    ...
    print(f"      articles={len(articles)} usable_materials={len(materials)}", flush=True)
    if not materials:
        raise SystemExit("no usable abstract-level materials fetched from PubMed")
    ...
    outcome_determinability.setdefault("death", {"outcome": "death", "status": "known_0_1"})
    ...
    print("[4/5] PubMed prior-art client (novelty; no OpenRouter screen) ...", flush=True)
    ...
    print("[5/5] running idea-mining dry run (S4->S1->S3->S2, stop at gate) ...", flush=True)
