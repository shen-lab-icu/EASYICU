...
synthetic ICU cohort with a *component-completeness* signal and runs
the full agent pipeline against it using the
...
The synthetic cohort is intentionally constructed so that:

1. SOFA-2 increases mortality monotonically; a small genuinely
   low-severity tail sits at score 0 (a real low score, not an
   artefact). Separately, ~10% of patients are component-under-measured
   (recorded via ``sofa2_n_components``); they keep their real score
   but carry hidden severity (legitimate MNAR). This exercises the
   component-completeness QC the agent layer enforces — distinguishing
   structural under-measurement from a genuinely low score, without
   collapsing missing components into a spurious 0.
...
def build_synthetic_cohort(n: int = 1500, seed: int = 7) -> pd.DataFrame:
    """Build a small synthetic ICU cohort with a component-completeness signal."""
    rng = np.random.default_rng(seed)

    age = rng.normal(loc=65, scale=15, size=n).clip(18, 95)
    sex = rng.choice(["M", "F"], size=n, p=[0.55, 0.45])

    # Most patients get a SOFA2 in 1..14.
    base_sofa2 = rng.integers(low=1, high=14, size=n, endpoint=False)

    # ~5% are a genuine low-severity tail at score 0 — a REAL low score,
    # not an artefact. Separately ~10% are component-under-measured: they
    # KEEP their real score (we do NOT collapse missing components into a
    # spurious 0), and the completeness is carried in ``sofa2_n_components``
    # so the QC can flag under-measurement without conflating it with a
    # low score.
    truly_low = rng.random(n) < 0.05
    missing_components = rng.random(n) < 0.10
    sofa2 = np.where(truly_low, 0, base_sofa2)

    # Mortality rises with SOFA2 (logit). Under-measured patients carry
    # hidden severity (legitimate MNAR), independent of the score value.
    logit = -3.5 + 0.18 * sofa2 + 0.012 * (age - 65)
    logit += np.where(missing_components, 1.5, 0.0)  # hidden severity (MNAR)
    ...
    return pd.DataFrame({
        ...
        "sex": sex,
        "sofa2": sofa2,
        "sofa2_n_components": np.where(missing_components, 0, 6),
        "lact": lact,
        "creat": creat,
        "map": map_mmhg,
        "vaso": vaso,
...
def main() -> int:
    ...
    result = pipeline.run(
        ...
        notes=(
            ...
            "SOFA-2 score 0 is a genuine low score, and component under-measurement is "
            "carried separately in sofa2_n_components to exercise completeness QC."
        ),
    )
