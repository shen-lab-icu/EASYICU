...
| `execution/runners/prediction_validation_executor.py` | 347 | `experimental` | prediction-validation | Direct incubator APIs consume an explicit typed contract and supplied probabilities; the CSV route binds exact UTF-8 source bytes, SHA-256, a versioned strict-parser profile, contract and result into a non-authorizing receipt, while validators fully recompute candidates. Duplicate/non-canonical headers, implicit bool/string numeric coercion, non-finite optional metrics, and invalid unvalidated model copies fail closed. Frozen and live base-R oracles cover AUROC, Brier score, non-identity joint calibration coefficients and threshold counts. It remains outside Planner/runtime selection and EvidenceStore, trains no model, and requires independent human review plus governed route/code authority before any paper claim. | 38 | - | 2026-10-01 |
