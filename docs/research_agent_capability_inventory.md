...
| `execution/runners/prediction_validation_executor.py` | 294 | `experimental` | prediction-validation | Direct incubator APIs consume an explicit typed contract and supplied probabilities; the CSV route binds exact UTF-8 source bytes, SHA-256, parser metadata, contract and result into a non-authorizing receipt, while validators fully recompute candidates. Frozen and live base-R oracles cover AUROC, Brier score, joint calibration coefficients and threshold counts. It remains outside Planner/runtime selection and EvidenceStore, trains no model, and requires independent human review plus governed route/code authority before any paper claim. | 29 | - | 2026-10-01 |
