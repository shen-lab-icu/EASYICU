...
| `acquisition/foundation.py` | 603 | `production_reachable` | acquisition | Public Web `make_research_pipeline_run_runner` → returned runner → `acquire_universe_for_question` → `ResearchAgentPipeline.run`; the host owns pre-sandbox acquisition. | `tests/test_pi_copilot_research_workflow.py::test_web_runner_delegates_to_research_agent_pipeline` | `call:make_research_pipeline_run_runner;call:runner;trace:acquire;trace:run` | 2026-11-01 |
