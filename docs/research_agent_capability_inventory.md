...
| `execution/runners/prediction_validation_executor.py` | 406 | `experimental` | prediction-validation | Direct incubator APIs consume an explicit typed contract and supplied probabilities; the CSV route binds exact UTF-8 source bytes, SHA-256, a versioned strict-parser profile, contract and result into a non-authorizing receipt, while validators fully recompute candidates and can issue a runtime-bound zero-finding host seal. Duplicate/non-canonical headers, implicit bool/string numeric coercion, non-finite optional metrics, and invalid unvalidated model copies fail closed. Frozen and live base-R oracles cover AUROC, Brier score, non-identity joint calibration coefficients and threshold counts. It remains outside Planner/runtime selection, trains no model, and cannot itself register EvidenceStore or paper authority. | 48 | - | 2026-10-01 |
| `authority/prediction_validation_evidence.py` | 511 | `experimental` | prediction-validation | Direct incubator bridge resolves seven exact, digest-verified upstream EvidenceStore records (prediction table, cohort, subject-disjoint split, model, source snapshot, environment lock and runtime receipt), binds them to a host-recomputed receipt, and registers one deterministic `analysis_only` bundle. Registration publishes no aliases, numeric claims or scientific claims; revalidation detects later authority promotion. It remains outside Planner/runtime selection and grants no paper authority. | 10 | - | 2026-10-01 |
