...
| `execution/runners/prediction_validation_executor.py` | 115 | `experimental` | prediction-validation | Direct incubator API `run_prediction_validation` consumes an explicit typed contract and supplied probabilities, while `prediction_validation_result_findings` recomputes and rejects altered results. It is not registered in Planner or runtime selection, writes no EvidenceStore authority, trains no model, and cannot support a paper claim until a governed product route and independent oracle review are approved. | 20 | - | 2026-10-01 |
