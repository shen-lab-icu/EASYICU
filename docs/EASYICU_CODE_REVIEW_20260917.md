...
此前四轮报告覆盖的是修复过程中的源码快照。本次以 `origin/main=f845bc2a9`、原 PR HEAD `b2f059768` 为基线，复核 974 文件汇总差异、原报告的关键修复路径、当前 CI 收据和敏感文件清单；不是重新声称逐行精读全部 974 文件。发现两处可复现缺陷，已分批修复：
...
聚焦验证：缓存及相邻数据合同 **51 passed**，护栏 **10 passed**，改动文件 Ruff、Black 和 `git diff --check` 通过。完整 exact-head CI 必须以本次新增提交后的最终 SHA 与 PR 检查为准，不能沿用原 `b2f059768` 收据。现存 Linux GTK/WebKit `glib 0.18.5` 告警以及发布前依赖锁审计仍单独保留；工程合并不授予研究启动或论文权限。
