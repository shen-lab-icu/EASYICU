...
## PR #15 当前提交复核补记（2026-09-18）

此前四轮报告覆盖的是修复过程中的源码快照。本次以 `origin/main=f845bc2a9`、原 PR HEAD `b2f059768` 为基线，复核 974 文件汇总差异、原报告的关键修复路径、当前 CI 收据和敏感文件清单；不是重新声称逐行精读全部 974 文件。发现四处可复现缺陷，已分批修复：

1. **签名 pickle 缓存可跨请求重放，写入时可能认证竞争替换**。原 HMAC 只覆盖 pickle 字节，不绑定由请求参数生成的缓存键。合成数据实测：把 `sbp` 的 payload 和签名复制到 `hr` 文件名下，`hr` 缓存错误地返回 `sbp` 结果，且不重新提取。写入路径还在落盘后重新读取共享文件签名，竞争替换可被认证。`f33f7e450` 让 HMAC 带版本与缓存文件名，并直接签本进程序列化的字节；两项回归测试先在旧实现红、修后绿。旧格式签名会失败关闭并重新提取，不继续信任旧缓存。
...
4. **暂存图件修复会搬移符号链接指向的外部目录文件**。`execution/phase.py::_repair_publication_figure_in_staging` 原来在创建备份后遍历 `out_dir.iterdir()`，若代理把整个 `outputs` 换成指向外部目录的链接，宿主会把外部文件搬进临时备份并删除。合成回归在旧实现未拒绝链接，`c9bc72c59` 在暂存前和正式安装前校验输出路径位于真实运行目录内，拒绝链接；修后外部哨兵文件不变。架构体积基线明确记录执行文件增加 3 行，没有隐藏这项安全增量。
...
`77219f8ed` 的远端研究代理检查随后得到 **2614 passed、1 failed**：执行层从 `publication_bundles` 导入新的私有路径函数，触犯仓库已固定的跨模块公开接口合同。这是实际 CI 失败，未按通过处理。`53476f92f` 将路径校验变为 `publication_filesystem` 的公开 owner 接口，执行层直接导入该接口；体积无新增增长，模块图仅增加一条无环依赖边。原失败测试与相邻合同 **124 passed**，选定流水线集成 **4 passed**，架构、模块图和导入边界 **7/7** 均通过。

其余聚焦验证：缓存及相邻数据合同 **51 passed**，护栏 **10 passed**，发布路径回归 **8 passed**；改动文件 Ruff、Black 和 `git diff --check` 通过。`6ac41fbac`、`17605aa88`、`b21f08368` 与 `77219f8ed` 的全矩阵因后续发现或检查失败而取消，完整 exact-head CI 必须以最终 SHA 与 PR 检查为准，不能沿用旧 `b2f059768` 或取消的收据。现存 Linux GTK/WebKit `glib 0.18.5` 告警以及发布前依赖锁审计仍单独保留；工程合并不授予研究启动或论文权限。
