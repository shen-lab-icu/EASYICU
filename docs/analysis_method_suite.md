...
| `time_to_event.competing_risks_cif` | `competing_risks`, `gray_test` | `lifelines` |
...
| `phenotyping.early_subtype_assignment` | `subtype_assignment` | `sklearn` |
...
| Aalen-Johansen competing-risk CIF + Gray comparison | standard_supporting | LLM-coded ⚠️ | cause-specific CIF curves + descriptive group difference + Gray test | — |
...
| Propensity-score matching / stabilised weighting diagnostics | standard_supporting | LLM-coded ⚠️ | propensity scores + matched pairs or stabilised weights + SMD balance table | — |
...
| Doubly-robust AIPTW sensitivity | exploratory | LLM-coded ⚠️ | AIPTW ATE + influence-function SE/CI + positivity diagnostics | — |
| Product-method mediation sensitivity | exploratory | LLM-coded ⚠️ | natural direct/indirect/total effect table + bootstrap intervals | — |
...
| Restricted cubic spline dose-response (continuous exposure) | standard_supporting | LLM-coded ⚠️ | spline_dose_response.csv (x, log_or, ci) + spline panel | — |
...
| Net reclassification / IDI vs a baseline model | exploratory | LLM-coded ⚠️ | bootstrap categorical/continuous NRI + IDI table | — |
...
| Early-feature assignment to frozen trajectory subtypes | exploratory | LLM-coded ⚠️ | held-out subtype probabilities + balanced accuracy/macro-F1/log-loss/multiclass-Brier + calibration/confusion/coefficient tables | — |
