"""Project-level configuration for easyicu.
...
# Default log level
LOG_LEVEL = os.getenv('EASYICU_LOG_LEVEL', 'INFO')
...
CACHE_SIZE_LIMIT = int(os.getenv('EASYICU_CACHE_SIZE_MB', '0'))  # 0 = unlimited cache by default

# Auto-clear cache on startup - helps prevent stale data issues
# 🚀 性能优化：默认禁用自动缓存清除，避免每次导入都清除缓存
# 如需清除缓存，可设置环境变量 EASYICU_AUTO_CLEAR_CACHE=True 或调用 clear_cache()
AUTO_CLEAR_CACHE = os.getenv('EASYICU_AUTO_CLEAR_CACHE', 'False').lower() in ('true', '1', 'yes')

# Number of parallel workers - optimized for 16GB systems
MAX_WORKERS = int(os.getenv('EASYICU_MAX_WORKERS', '999'))  # Reduced from 4 to 2 for memory efficiency

# Chunk size for batch processing - optimized for 16GB systems
CHUNK_SIZE_STR = os.getenv('EASYICU_CHUNK_SIZE', 'None')
...
AUTO_OPTIMIZE_DTYPES = os.getenv('EASYICU_AUTO_OPTIMIZE_DTYPES', 'True').lower() in ('true', '1', 'yes')
ENABLE_MEMORY_MONITORING = os.getenv('EASYICU_ENABLE_MEMORY_MONITORING', 'True').lower() in ('true', '1', 'yes')
USE_CHUNKED_LOADING = os.getenv('EASYICU_USE_CHUNKED_LOADING', 'auto')  # auto, always, never
