...
def _typed_input_scope_contract(step: AnalysisStep) -> str:
    ...
    return (
        ...
        "- A producer row that publishes partition_identities states the only sums "
        "its count columns satisfy, and the row's own *_pct columns already carry "
        "the denominator chosen for that question. Read both from the bound rows: "
        "never pick among overlapping count columns, never divide a hand-picked sum "
        "by n_total, and never require a set the row does not state to add up.\n"
