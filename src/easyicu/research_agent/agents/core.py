...
def _typed_input_scope_contract(step: AnalysisStep) -> str:
    ...
    return (
        ...
        "describe that same digest-bound file. These are schema facts only and "
        ...
        "- Numeric coercion must fail closed: after any "
        "`pd.to_numeric(..., errors='coerce')`, count the newly invalidated "
        "values (`(original.notna() & coerced.isna()).sum()`) and raise when "
        "that count is positive (or import the host helper and use "
        "`strict_numeric_input(original).values`). Never gate a domain check on `.notna()` "
        "alone without that upstream guard — coerced dirty values would "
        "silently become missingness.\n"
