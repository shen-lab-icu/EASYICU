    ...
    def _prompt(
        ...
        *,
        ...
        return (
            ...
            "Measurement-count columns may be independent QC fields: do not "
            "require counts to define the analytic exposure or source-status "
            "mask when the script separately compares (count > 0) with the "
            "measured flag, reports discordance or unavailable count columns, "
            "and keeps that comparison audit-only. Flag count handling only "
            "when available counts are ignored or contradictions are silently "
            "accepted. "
