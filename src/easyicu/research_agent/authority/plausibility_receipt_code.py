"""Host-rendered plausibility receipt code and its exact source reader.

The authority layer decides *which* exact raw columns owe a receipt.  The
sandbox source that computes it, and the text-only check that a submitted
script contains that exact generated tail, live here in the leaf layer so the
audit and execution layers can both use them without auditing code importing
...
compared against the bound.  ``observed_n - compared_n`` is therefore already
the count of values that were present but would not convert, and
``coercion_loss_n`` states it instead of making a reader do the subtraction.
The delivered ``plausibility_audit`` object stays plain-named because only it
is followed by name into the summary write.
"""
...
from .plausibility import FlagOnlyPlausibilityScope
...
def render_host_plausibility_receipt_tail(scope: FlagOnlyPlausibilityScope) -> str:
...
def verified_host_plausibility_receipt_region(
    ...
    try:
        ...
        return None
    tail = render_host_plausibility_receipt_tail(scope)
