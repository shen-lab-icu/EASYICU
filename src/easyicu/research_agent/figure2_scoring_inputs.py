...
The paper evaluator must not read mutable run-root files.  This module binds
one selected run checkpoint to one selected EvidenceStore generation, verifies
the complete evidence-coordinate multiset, and materializes every scorer input
from digest-verified current evidence.  It deliberately owns no run lock: the
benchmark orchestrator is the sole lock owner.
"""
...
FIGURE2_SCORING_INPUT_AUTHORITY_SCHEMA = "easyicu.figure2_scoring_input_authority/3"
FIGURE2_RUN_TASK_AUTHORITY_SCHEMA = "easyicu.figure2_run_task_authority/1"
...
class Figure2RunTaskAuthority(_StrictFrozenModel):
    ...
    schema_version: Literal["easyicu.figure2_run_task_authority/1"]
    ...
    exposure_concept: str | None = Field(default=None, min_length=1, max_length=256)
    outcome_concept: str | None = Field(default=None, min_length=1, max_length=256)
    evidence_generation: int = Field(ge=1)
    evidence_payload_sha256: Sha256

    @field_validator("exposure_concept", "outcome_concept")
    @classmethod
    def _nonblank_optional_concept(cls, value: str | None) -> str | None:
        if value is not None and not value.strip():
            raise ValueError("task concept coordinates must be nonblank when present")
        return value
...
class Figure2ScoringInputAuthority(_StrictFrozenModel):
    ...
    schema_version: Literal["easyicu.figure2_scoring_input_authority/3"]
    task_id: str = Field(min_length=1, max_length=128)
    suite_ref: Literal["easyicu_evaluation_protocol_suite/v2"]
    suite_projection_sha256: Sha256
    paper_rubric_ref: Literal["easyicu.figure2_paper_rubric/20260718-v2"]
...
def load_figure2_scoring_inputs(
    ...
    *,
    ...
    """Load one fully bound scorer input set and evaluator review corpus."""
    ...
    root = Path(run_dir)
    checkpoint = load_run_artifact_authority(root)
    if checkpoint is None:
        raise LookupError("run has no modern checkpoint authority")
    checkpoint_bytes = _canonical_json_bytes(checkpoint)
    run_id = checkpoint.get("run_id")
    sequence = checkpoint.get("checkpoint_sequence")
    if not isinstance(run_id, str) or not run_id.strip():
        raise ValueError("current checkpoint lacks a valid run_id")
    if type(sequence) is not int or sequence < 1:
        raise ValueError("current checkpoint lacks a valid checkpoint_sequence")
    research_question = checkpoint.get("research_question")
    question_sha256 = _research_question_sha256(research_question)
    task_authority = Figure2RunTaskAuthority.model_validate(
        checkpoint.get("figure2_task_authority"), strict=True
    )
    if task_authority.task_id != expected_task_id:
        raise Figure2TaskAuthorityMismatch(
            "checkpoint Figure 2 task does not match requested task"
        )
    if task_authority.suite_projection_sha256 != figure2_suite_projection_sha256():
        raise OSError("checkpoint Figure 2 suite projection has drifted")
    if task_authority.research_question_sha256 != question_sha256:
        raise OSError("checkpoint Figure 2 research question has drifted")
    if task_authority.research_question_sha256 != _frozen_task_objective_sha256(
        expected_task_id
    ):
        raise OSError(
            "checkpoint Figure 2 research question does not match the frozen task "
            "objective"
        )
    paper_rubric = load_figure2_paper_rubric()
    if task_authority.paper_rubric_ref != FIGURE2_PAPER_RUBRIC_REF or (
        task_authority.paper_rubric_sha256 != paper_rubric_manifest_sha256(paper_rubric)
    ):
        raise OSError("checkpoint Figure 2 paper rubric has drifted")
    readiness = checkpoint.get("readiness")
    raw_steps = checkpoint.get("per_step_records")
    raw_records = checkpoint.get("evidence")
    if not isinstance(readiness, dict) or readiness.get("manuscript_ready") is not True:
        raise PermissionError("current checkpoint is not manuscript_ready")
    ...
    if (
        task_authority.evidence_generation != snapshot.generation
        or task_authority.evidence_payload_sha256 != snapshot.payload_sha256
    ):
        raise OSError(
            "checkpoint Figure 2 task authority selects a different "
            "EvidenceStore generation"
        )

    checkpoint_records = [dict(item) for item in raw_records]
    evidence_records = [dict(item) for item in snapshot.records]
    checkpoint_coordinates = Counter(
        _record_coordinate(item) for item in checkpoint_records
    )
    ...
    if checkpoint_coordinates != evidence_coordinates:
        raise OSError(
            "checkpoint and current EvidenceStore generation contain different evidence coordinates"
        )
