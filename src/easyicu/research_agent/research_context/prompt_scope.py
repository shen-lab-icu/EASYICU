...
- Use only the exact digest-bound typed inputs in `EASYICU_RESOLVED_INPUTS_JSON`. Read it as a JSON-file path; verify each evidence id/path/digest and product schema. Never scan run directories, reuse old figures, rank records, or choose columns by dtype/position.
...
- Write `<stem>_source_data.csv` (or multiple CSVs) with every plotted value, denominator, source table, and reconciliation key. Keep each typed parent's original value-column names in a separate exact/subset CSV; do not rename unrelated measures to generic `value`, `numerator`, or `denominator` columns. Exclude unplotted masks, duplicate rounded values, and unauthorised derivations. Positional traces use `source_row_index` and `source_table`.
...
- Use matplotlib `Agg` and editable SVG text. Build all exports from one figure/source dataset with `make_figure_contract`, `apply_publication_style`, `add_panel_label`, `save_publication_figure`, and `audit_publication_exports_json` from `easyicu.research_agent.figures.publication`. Call `save_publication_figure(fig=fig, out_dir=out_dir, stem=stem, contract=contract)` directly and save matching PNG, SVG, PDF, and TIFF files. It writes `<stem>.figure_contract.json`; never write that file yourself. `FigureContract` is a pydantic model: hand-rolled `json.dump(contract, ...)` can raise `Circular reference detected` and leave a fatal 0-byte file.
- Use the stable FigureContract fields `figure_id`, `core_claim`, `panels`, `source_data`, and `reader_caption`; each panel uses `panel_id`, `role`, `claim`, `evidence_ids`, plus `chart_type` or `visual_form`. `source_data` is one local CSV basename or a flat list of basenames; evidence ids belong on panels.
...
- Keep labels reader-facing and compact. Use `constrained_layout=True` or explicit GridSpec spacing; keep labels, legends, and value text within their panels. Use compact unique panel labels, no duplicated label in a title, and no figure-level title, caption, or long note on the canvas; the legend belongs in `reader_caption`.
...
- For exactly two declared groups, the SDK output includes comparison-minus-reference standardized differences for every continuous row and categorical level; preserve those fields unchanged.
...
- Measured/count/status companions are audit-only. Run the exact host `measurement_provenance_receipt` API only for pairs whose measured flag is explicitly listed in this Planner step's `inputs`; do not add a pair merely because a value column's raw contract exposes companion fields. Publish the unchanged records only as `step_summary["measurement_provenance_audit"]={"source":"COHORT_PARQUET","checks":[...]}`. The helper itself accepts no `source` argument and its returned records have no `source` field. Fail the whole step on discordance; never use a companion to mask, filter, relabel, impute, or change the authoritative value denominator.
