    ...
    for attempt in range(2):
        ...
        for spec, error_detail in repair_specs:
            repair_instruction = (
                ...
                + "its database column). Do not write `point estimate` or a numeric "
                + "`robustness` summary unless that sentence names the exact digest "
                + "metric; otherwise omit it. Remove backticked underscore identifiers "
                + "and every engineering phrase named above. Preserve required headings."
                + (
