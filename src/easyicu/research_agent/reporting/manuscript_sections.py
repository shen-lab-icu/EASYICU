    ...
    for attempt in range(2):
        ...
        for spec, error_detail in repair_specs:
            repair_instruction = (
                ...
                + "its database column). Never write the literal phrase `point "
                + "estimate`, or a numeric `robustness` value/range, unless the same "
                + "sentence names the exact statistic from the machine digest. If "
                + "the exact metric is unavailable, omit that sentence. Before "
                + "returning, scan the complete section and remove every backticked "
                + "underscore identifier, engineering phrase named in the findings, "
                + "and unnamed numeric summary. Preserve all required headings and "
                + "labels."
                + (
