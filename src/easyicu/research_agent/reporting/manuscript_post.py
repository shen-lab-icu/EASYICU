...
_PRIMARY_ASSOCIATION_SECTION_RE = re.compile(
    r"(?ms)(^### Primary association\s*\n)(.*?)(?=^### |^## |\Z)"
)
_ABSTRACT_RESULTS_LABEL_RE = re.compile(
    r"(?ms)(^\*\*Results:\*\*\s*)(.*?)"
    r"(?=^\*\*(?:Background|Methods|Conclusions):\*\*|^## |\Z)"
)


def repair_missing_reportable_survival_results(
    scaffold: str,
    *,
    per_step_records: Sequence[Mapping[str, Any]],
) -> tuple[str, List[Dict[str, Any]]]:
    """Project a complete owner-issued RMST contrast when Writer omitted it.

    The survival executor, not this renderer, owns the estimand and values.  A
    repair is allowed only for the typed non-PH reporting contract and only
    when the exact owning statistic evidence id is present on the same current
    step record.  The ordinary strict numeric-binding pass still revalidates
    every projected number after this repair.
    """

    match = _PRIMARY_ASSOCIATION_SECTION_RE.search(scaffold)
    if match is None:
        return scaffold, []
    primary_body = match.group(2)
    for record in per_step_records:
        ...
        if not isinstance(reporting, Mapping):
            continue
        if reporting.get("schema_version") != "easyicu.survival_reporting/1":
            continue
        if reporting.get("execution_owner") != "landmark_survival_executor_v1":
            continue
        if reporting.get("constant_hazard_ratio_authorized") is not False:
            continue
        if not str(reporting.get("proportional_hazards_status") or "").startswith(
            "violation_"
        ):
            continue
        rmst = reporting.get("rmst")
        if not isinstance(rmst, Mapping):
            continue
        ...
        try:
            values = {
                "tau": float(rmst["tau_days_from_landmark"]),
                "exposed": float(rmst["exposed_rmst_days"]),
                "comparator": float(rmst["comparator_rmst_days"]),
                "difference": float(rmst["difference_days"]),
                "ci_low": float(rmst["ci_low"]),
                "ci_high": float(rmst["ci_high"]),
                "p_value": float(rmst["p_value"]),
            }
        except (KeyError, TypeError, ValueError):
            continue
        required_literals = (
            f"{values['exposed']:.3f}",
            f"{values['comparator']:.3f}",
            f"{values['difference']:.3f}",
            f"{values['ci_low']:.3f}",
            f"{values['ci_high']:.3f}",
        )
        if all(literal in primary_body for literal in required_literals):
            return scaffold, []
        method = str(rmst.get("method") or "").strip()
        method_label = {
            "unadjusted_kaplan_meier_plugin": "unadjusted Kaplan–Meier plug-in",
        }.get(method, method.replace("_", " ") or "owner-specified")
        sentence = (
            f"Through {values['tau']:.0f} days after the landmark, the "
            f"{method_label} restricted mean survival time was "
            f"{values['exposed']:.3f} days in the exposed group and "
            f"{values['comparator']:.3f} days in the comparator group, a signed "
            f"difference of {values['difference']:.3f} days (95% CI, "
            f"{values['ci_low']:.3f} to {values['ci_high']:.3f}; "
            f"p = {values['p_value']:.2e}) "
            f"{{evidence:{evidence_id}}}."
        )
        replacement = match.group(1) + primary_body.rstrip() + "\n\n" + sentence + "\n\n"
        repaired = scaffold[: match.start()] + replacement + scaffold[match.end() :]
        abstract_match = _ABSTRACT_RESULTS_LABEL_RE.search(repaired)
        if abstract_match is not None and not all(
            literal in abstract_match.group(2) for literal in required_literals
        ):
            abstract_sentence = (
                f"The unadjusted {values['tau']:.0f}-day restricted mean survival "
                f"time was {values['exposed']:.3f} versus "
                f"{values['comparator']:.3f} days (signed difference, "
                f"{values['difference']:.3f} days; 95% CI, "
                f"{values['ci_low']:.3f} to {values['ci_high']:.3f}; "
                f"p = {values['p_value']:.2e}) "
                f"{{evidence:{evidence_id}}}. "
            )
            repaired = (
                repaired[: abstract_match.start()]
                + abstract_match.group(1)
                + abstract_sentence
                + abstract_match.group(2).lstrip()
                + repaired[abstract_match.end() :]
            )
        return repaired, [
            {
                "reason_code": "owner_reportable_survival_rmst_projected",
