...
BenchTaskKind = Literal[
    ...
    # General ICU research method families used by external evaluation suites.
        ...
        tasks=[
            ...
            ICUAgentBenchTask(
                ...
                semantic_guardrails=[
                    ...
                    "Audit component completeness before drawing conclusions from composite scores.",
                ],
            ),
            ICUAgentBenchTask(
            ...
            ICUAgentBenchTask(
                ...
                gold_answer=ICUAgentBenchGoldAnswer(
                    ...
                    numeric_targets={
                        ...
                        "sofa2_low_component_frac": ICUAgentBenchNumericBound(
                            lower=0.08, upper=0.11
                        ),
                        "sofa2_mean": ICUAgentBenchNumericBound(lower=6.5, upper=7.1),
            ...
            ICUAgentBenchTask(
                ...
                kind="survival_analysis",
                category="self_check",
                title="Stratified SOFA-2 mortality gradient",
                objective=(
                    "Demonstrate a monotonically increasing mortality signal "
                    "across SOFA-2 tertiles over the full cohort. This task "
                    "locks the stratified SOFA-mortality gradient; the delta "
                    "target guards against a model that erases stratification "
                    "by averaging across the whole cohort."
                ),
                expected_outputs=[
                    "stratified mortality table",
                    "tertile boundary definitions",
                ],
                semantic_guardrails=[
                    "Report tertile boundaries explicitly, not magic numbers.",
                    "Treat a SOFA-2 of 0 as a real low score, not as missing.",
                ],
                target_databases=[],
                ...
                gold_answer=ICUAgentBenchGoldAnswer(
                    data_fixture="synthetic_cohort",
                    numeric_targets={
                        "death_low_tertile": ICUAgentBenchNumericBound(
                            lower=0.03, upper=0.09
                        ),
                        "death_high_tertile": ICUAgentBenchNumericBound(
                    ...
                    required_warnings=[],
                    forbidden_outputs=[
                        "treat sofa2==0 as missing",
                    ],
                    derivation=(
                        "Bounds from the synthetic_cohort fixture, tertile-"
                        "splitting by SOFA-2 quantiles over the full cohort. "
                        "The delta-hi-minus-lo target guards against a model "
                        "that erases stratification by averaging across the "
                        "whole cohort."
                    ),
                ),
            ),
    ...
    * ``"self_check"`` results feed ``self_check_aggregate`` — a
    ...
    def _mean_over(bucket: List[ICUAgentBenchTaskResult]) -> Dict[str, float]:
        ...
        for name in metric_names:
            values = [getattr(r, name) for r in bucket if getattr(r, name) is not None]
            if values:
    ...
    * Without a ``suite`` argument every task is treated as
    ...
    if corr is None:
        ...
        eval_unchanged = sum(
            1
            for d in deltas
            if d.metric == "correctness" and d.category == "evaluation" and d.delta == 0
        )
        verdict = (
