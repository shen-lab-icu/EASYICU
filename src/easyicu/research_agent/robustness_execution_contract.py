...
ROBUSTNESS_EXECUTION_CONTRACT_GUIDANCE = (
    ...
    "the modeled value. When both an authoritative measured/source-status flag "
    "and a companion observation count are available, reconcile them before "
    "applying the membership predicate and report invalid and discordant pair "
    "counts. Do not silently accept a row whose flag and count disagree; if a "
    "membership-changing disagreement cannot be resolved from registered raw "
    "observations, mark that specification not_executable. For a missing-"
    "indicator specification, derive measurement availability from the "
    "designated measured/source-status field when one is declared; do not infer "
    "it only from isna() on the modeled value. A numeric fill value may be used "
    "only as a documented computational encoding alongside that indicator, "
    "never as an observed physiologic value. Preserve missing or unrecognised "
    "categorical covariate levels: do not silently map them into the reference "
    "category; use the planned explicit category or missing-row policy. Cohort "
    "membership N and fitted analytic n are distinct quantities. Also emit one "
    "unique schema-identifiable model "
