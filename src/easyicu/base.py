"""
Base classes and utilities for easyicu
...
class BaseICULoader:
    ...
    def _setup_data_path(self, data_path: Optional[Union[str, Path]], database: str) -> Path:
        ...
        if easyicu_data_path:
            base_path = Path(easyicu_data_path)
            # 数据库名称到目录名的映射
            db_dir_mapping = {
            ...
            for subdir in db_dir_mapping.get(database, [database]):
                ...
                if candidate.is_dir():
                    if self.verbose:
                        logger.info(f"Using path from EASYICU_DATA_PATH: {candidate}")
                    return candidate
        ...
        try:
            ...
            if not is_ready:
                ...
                logger.warning(
                    ...
                    f"   Run: python -m easyicu.data_converter {self.data_path}\n"
    ...
    def _init_concept_system(self, dict_path: Optional[Union[str, Path, List[Union[str, Path]]]], use_sofa2: bool):
        ...
        try:
            if dict_path is None:
                env_override = os.getenv("EASYICU_DICT_PATH") or os.getenv("EASYICU_DICT_DIR")
                if env_override:
    ...
    def _resolve_parallel_workers(self, requested: Optional[int]) -> int:
        ...
        env_value = os.getenv("EASYICU_PARALLEL_WORKERS")
        if env_value:
            try:
                ...
                logger.warning("Invalid EASYICU_PARALLEL_WORKERS=%s, ignoring", env_value)

        # ⚡ 默认单线程以避免GIL竞争和锁开销
        # 用户可通过环境变量EASYICU_PARALLEL_WORKERS或参数显式启用并行
        default_workers = 1
    ...
    def _resolve_parallel_backend(self, backend: Optional[str]) -> str:
        ...
        env_backend = os.getenv("EASYICU_PARALLEL_BACKEND")
        if env_backend:
    ...
    def load_concepts(
        ...
        merge: bool = True,
        r_compatible: bool = True,  # 默认启用ricu.R兼容模式
    ...
    def _load_concepts_once(
        ...
        try:
            ...
            # Compatibility fix: when running in r_compatible mode, R ricu
            ...
            # are present.
            try:
                if r_compatible and isinstance(result, pd.DataFrame):
