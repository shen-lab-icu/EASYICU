...
def setup_theme(theme: str = 'easyicu') -> None:
    """设置 Plotly 主题。
    
    Args:
        theme: 主题名称 ('easyicu', 'plotly', 'plotly_white', 'plotly_dark')
    """
    if not HAS_PLOTLY:
    ...
    if theme == 'easyicu':
        # 自定义 EasyICU 主题
        pio.templates['easyicu'] = go.layout.Template(
            layout=go.Layout(
...
def get_concept_color(concept: str) -> str:
    """获取概念的颜色。"""
    return EASYICU_COLORS.get(concept, EASYICU_COLORS['primary'])
