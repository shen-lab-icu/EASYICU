...
def _prime_summary_draft_setup(state: dict[str, Any]) -> None:
    ...
    if run_dir_text:
        ...
        try:
            st.session_state["research_agent_workdir"] = str(Path(run_dir_text).expanduser().resolve().parent)
        except Exception:
            st.session_state["research_agent_workdir"] = str(Path(run_dir_text).expanduser().parent)
