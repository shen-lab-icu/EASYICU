...
# --------------------------------------------------------------------------
# Data-class schema (extracted to concept_schema.py 2026-05-17, Phase 1).
# Re-exported below so ``from easyicu.concept import ConceptSource`` etc.
# keeps working. Edit the schema in concept_schema.py, not here.
# --------------------------------------------------------------------------
from .concept_schema import (  # noqa: F401
    Concept,
    ConceptDefinition,
    ConceptDictionary,
    ConceptSource,
                            ...
                            if _idtbl_ids:
                                try:
                                    ...
                                    if _idtbl_bucket_dir is not None or _idtbl_flat_dir is not None:
                                        ...
                                        # Build WHERE
                                        # 🚀 perf B2: register itemid / patient_id filters as
                                        # DuckDB views rather than embedding thousands of
                                        # literals in the SQL IN list. Inline lists were the
                                        # main reason cohort-wide id_tbl loads on hirid /
                                        # eicu / chartevents had slow query plans.
                                        _idtbl_views: List[str] = []
                                        _is_str = any(isinstance(x, str) for x in _idtbl_ids)
                                        if len(_idtbl_ids) > 32:
                                            _idtbl_id_view = "_easyicu_idtbl_ids"
                                            _idtbl_conn.register(
                                                _idtbl_id_view,
                                                pd.DataFrame({_idtbl_sub_var: list(_idtbl_ids)}),
                                            )
                                            _idtbl_views.append(_idtbl_id_view)
                                            _idtbl_where = f"{_idtbl_sub_var} IN (SELECT {_idtbl_sub_var} FROM {_idtbl_id_view})"
                                        else:
                                        ...
                                        if patient_ids:
                                            ...
                                            if len(_pids) > 32:
                                                _idtbl_pid_view = "_easyicu_idtbl_pids"
                                                _idtbl_conn.register(
                                                    _idtbl_pid_view,
                                                    pd.DataFrame({_idtbl_id_col: list(_pids)}),
                                                )
                                                _idtbl_views.append(_idtbl_pid_view)
                                                _idtbl_pid_filter = f"AND {_idtbl_id_col} IN (SELECT {_idtbl_id_col} FROM {_idtbl_pid_view})"
                                            else:
                                        ...
                                        try:
                                            ...
                                            for _v in _idtbl_views:
                                                try:
                                                    _idtbl_conn.unregister(_v)
                                                except Exception:
                                                    pass
                                        if len(frame) > 0:
                                            table = ICUTable(
        ...
        if interval is not None and index_column and index_column in combined.columns and not is_win_tbl_target and not is_id_tbl_target:
            ...
            if should_expand:
                ...
                try:
                    ...
                    # 🔧 2026-05-16: known issue — when start/end columns have
                    # mismatched dtypes (numeric hours vs datetime, mixed datetime
                    # precisions), expand() raises and we fall back to raw rows.
                    # The fallback is acceptable for boolean treatment-window
                    # concepts (delirium_tx, abx, etc.) since they don't strictly
                    # need hourly expansion to be informative. Log at INFO to
                    # avoid noise; bump to WARNING via EASYICU_VERBOSE_EXPAND=1.
                    import os as _os
                    if _os.environ.get('EASYICU_VERBOSE_EXPAND'):
                        logger.warning(f"Failed to expand win_tbl data for {concept_name}: {e}")
                    else:
                        logger.info(f"win_tbl expand skipped for {concept_name} (using raw rows): {e}")
                ...
                if hasattr(data_source, 'base_path') and data_source.base_path is not None:
                    ...
                    if index_path.is_file() and obs_path.is_dir():
                        try:
                            ...
                            if target_files:
                                ...
                                # 🚀 perf B1 (hirid LOS): targeted file list out of the
                                # same observation_tables converter run → shared schema.
                                # Drop union_by_name=true so DuckDB does not sweep file
                                # metadata for reconciliation.
                                read_expr = f"read_parquet([{file_list}])"
                    ...
                    if read_expr is None:
                        if bucket_path.is_dir():
                            ...
                            # 🚀 perf B1 (hirid LOS): same-dir parquets share schema.
                            read_expr = f"read_parquet('{glob_pattern}')"
...
# --------------------------------------------------------------------------
# Dictionary-loading shims (extracted to concept_loader.py 2026-05-17,
# Phase 1). Re-exported so ``from easyicu.concept import load_dictionary``
# etc. keeps working. The lru_cache on _load_concept_dict_cached lives in
# concept_loader; do not re-decorate here.
# --------------------------------------------------------------------------
from .concept_loader import (  # noqa: F401
    _get_concept_bounds,
...
# --------------------------------------------------------------------------
# Source-level callback dispatcher (extracted to concept_callback_apply.py
# 2026-05-17, Phase 2). Re-exported so existing callers — notably
# ``easyicu.base`` — keep working unchanged.
# --------------------------------------------------------------------------
from .concept_callback_apply import _apply_callback  # noqa: F401


# --------------------------------------------------------------------------
# R-style callback expression parsers (extracted to concept_expr_parser.py
# 2026-05-17, Phase 1). Re-exported below so callers can keep using the
# underscore names.
# --------------------------------------------------------------------------
from .concept_expr_parser import (  # noqa: F401, E402
    _apply_binary_op,
...
# --------------------------------------------------------------------------
# Public dictionary loader (extracted to concept_loader.py 2026-05-17,
# Phase 1). The ``Concept`` alias now lives in concept_schema and is
