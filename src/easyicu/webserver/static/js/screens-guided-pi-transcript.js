...
(function () {
  ...
  function create(host) {
    ...
    function hostActionFailed(actionCode, job, status) {
      ...
      if (actionCode === 'prepare_analysis_data') {
        return Boolean(job && job.gate_status === 'blocked' && !job.human_review_pending);
      }
      if (!PLAN_HOST_ACTION_CODES.has(String(actionCode || ''))) return false;
    ...
    function preparedDataAvailable(job, status) {
      // A diagnostic manifest and job completion do not prove extraction.
      // This enum comes from the host's typed-context/Parquet-footer check.
      return Boolean(job && job.research_input_state === 'prepared' && status === 'done'
    ...
    function hostActionCopy(actionCode, job, status, turn) {
      ...
      if (actionCode === 'prepare_analysis_data') {
        const refs = Array.isArray(job && job.artifact_refs) ? job.artifact_refs : [];
        const hasPlan = refs.some(ref => ref && ref.artifact === 'agent_plan.json');
        if (!preparedDataAvailable(job, normalizedStatus) || !hasPlan) {
          doneCopy = tr('The preparation task ended, but the available receipts do not confirm both a prepared dataset and an executable plan. Review the run status and evidence before continuing.', '数据准备任务已结束，但现有回执不足以确认分析数据和可执行计划均已就绪。请先核对运行状态与证据再继续。');
        }
      }
      return {
        user: copy.user,
    ...
    function hostJobProgressSteps(actionCode, job, copy, startedAt) {
      ...
      const failed = hostActionFailed(actionCode, job, jobStatus);
      if (actionCode === 'prepare_analysis_data' && preparedDataAvailable(job, jobStatus)) {
        steps.push({
      ...
      progress.forEach((row, index) => {
        ...
        steps.push({
          ...
          status: complete ? 'complete' : (row.status === 'failed' || failed ? 'error' : 'running'),
          at: startedAt + index + 2,
          text: complete ? stageDetails[step] || '' : '',
        });
      });
      return steps;
    }
      ...
      hostTurns.forEach((turn, turnIndex) => {
        ...
        if (status !== 'running') {
          messages.push({
            ...
            preparedDataAvailable: preparedDataAvailable(sourceJob || job, status),
