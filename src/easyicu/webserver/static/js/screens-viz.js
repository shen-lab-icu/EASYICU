...
(function () {
  ...
  function registerSourceFromInput(container, screenId, button) {
    ...
    window.EU_API.registerWorkspaceSource(path, { active: !multi, crossdb: true }).then(() => {
      setRouteError(screenId, null); window.EU_VIZ_WORKSPACE = null; window.EU_CROSSDB_WORKSPACE = null; window.EU_PATIENT_DRILLDOWN = null; window.EU_PATIENT_SOURCES = null; window.EU_COHORT_REVIEW = null; resetCohortFeatureSelection(); resetCrossSetupForSourceChange(); patientReview.resetForSourceChange(); repaintScreen(screenId);
    }).catch(err => {
  ...
  const vizContextOwner = window.EU_VIZ_CONTEXT_OWNER; if (vizContextOwner && typeof vizContextOwner.init === 'function') {
    vizContextOwner.init({
      activePath: registryActivePath, sources: registrySources, defaultExportPath, patient: patientReview.drilldown, cohort: cohortReview,
