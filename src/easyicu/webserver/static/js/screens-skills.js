...
(function () {
  ...
  const ui = { catalogMode: 'skills', source: 'all', category: 'all', query: '', hideDisabled: false,
    ...
    installEnabled: true, detail: null, loadingDetail: false, busy: false, error: '', notice: '' };
  ...
  const methodSkills = () => (Array.isArray(methodCapabilities().items) ? methodCapabilities().items : []).map(row => ({
    ...
    source: 'builtin', kind: 'method', enabled: !!row.enabled, toggleable: false,
  ...
  const publicationSkills = () => (Array.isArray(publicationCapabilities().items) ? publicationCapabilities().items : []).map(row => ({
    ...
    source: 'builtin', kind: 'publication', enabled: !!row.enabled, settingKey: row.setting_key, toggleable: true,
    version: row.version,
  ...
  const userSkills = () => (Array.isArray(registry().skills) ? registry().skills : []).map(row => ({
    ...row, id: `user:${row.name}`, title: row.name, category: row.category || ((row.stages || []).includes('writing')
      ? tr('Writing', '写作') : tr('Conversation', '对话')), source: 'user',
  }));
  ...
  function catalog() {
    const skillEntries = [...methodSkills(), ...publicationSkills(), ...userSkills()];
    ...
    const workflowCount = methodSkills().length;
    const componentCount = methodComponents().length;
    const plannedCount = Number(methodCapabilities().planned_method_count || 0);
    const publicationCount = publicationSkills().length;
    const groupSpecs = ui.catalogMode === 'methods'
      ...
      : ui.source === 'builtin' ? [
        ['workflow', 'builtin', tr('Research workflows', '研究工作流')],
        ['publication', 'builtin', tr('Writing and figures', '写作与图件')],
      ] : [
        ['workflow', 'builtin', tr('Research workflows', '研究工作流')],
        ['publication', 'builtin', tr('Writing and figures', '写作与图件')],
        ['user', 'user', tr('Created by you', '我创建的')],
      ];
    const cards = groupSpecs.map(([group, source, heading]) => {
      const rows = filtered.filter(item => item.source === source && (group === 'workflow' ? item.kind === 'method'
      ...
      return `<section class="eusk-group"><h2>${heading} <small>${rows.length}</small></h2>
        <div class="eusk-grid">${rows.map(item => `<article class="eusk-card ${item.enabled ? '' : 'disabled'}">
          ...
          <span class="eusk-source">${item.kind === 'method_component' ? tr('Method component', '方法组件') : sourceLabel(source)}</span><span class="eusk-state ${item.enabled ? 'on' : ''}">${item.enabled ? tr('Enabled', '已启用') : tr('Off', '未启用')}</span></div>
    ...
    return `<div class="eusk-head"><div><span class="eusk-eyebrow">SKILL HUB</span><h1>${tr('Skills', '技能')}</h1></div>
      ...
      <label class="eusk-search">${icon('search', 17)}<input data-sk-search type="search" placeholder="${ui.catalogMode === 'methods' ? tr('Search the method library…', '搜索方法库…') : tr('Search all your skills…', '搜索全部技能…')}" value="${esc(ui.query)}" autocomplete="off"></label>
      <div class="eusk-mode-tabs" role="tablist" aria-label="${tr('Catalog level', '目录层级')}">
        <button type="button" role="tab" aria-selected="${ui.catalogMode === 'skills'}" data-sk-mode="skills">${tr('Skills', '技能')} <span>${skillEntries.length}</span><small>${tr('Standalone tasks', '可独立启动')}</small></button>
        <button type="button" role="tab" aria-selected="${ui.catalogMode === 'methods'}" data-sk-mode="methods">${tr('Method library', '方法库')} <span>${methodEntries.length}</span><small>${tr('Analysis building blocks', '分析方法组件')}</small></button>
      </div>
      ...
      <div class="eusk-catalog-summary" aria-label="${tr('Capability catalogue summary', '能力目录摘要')}">
        ${ui.catalogMode === 'methods'
          ...
          : `<span>${tr(`${workflowCount} research workflows and ${publicationCount} writing or figure skills.`, `${workflowCount} 个研究工作流与 ${publicationCount} 个写作、图件技能。`)}</span><small>${tr(`Browse ${componentCount} analysis methods separately in the Method library.`, `${componentCount} 个分析方法在“方法库”中单独浏览。`)}</small>`}
      </div>
  ...
  function detail() {
    ...
    return `<button type="button" class="eusk-back" data-sk-back>← ${tr('Back to skills', '返回技能目录')}</button>
      <div class="eusk-detail-head"><div><span class="eusk-eyebrow">${esc(skill.category)} · ${sourceLabel(skill.source)}</span>
      ...
      <button type="button" class="eusk-use" data-sk-use="${esc(skill.id)}" ${skill.enabled ? '' : 'disabled'}>${method || (user && skill.enabled && skill.stages.includes('conversation')) ? tr('Use in new task', '在新任务中使用') : tr('Ask in new task', '在新任务中询问')} ↗</button>
      ...
      <div class="eusk-tabs" role="tablist"><button type="button" role="tab" aria-selected="${ui.tab === 'overview'}" data-sk-tab="overview">${tr('Overview', '概览')}</button><button type="button" role="tab" aria-selected="${ui.tab === 'files'}" data-sk-tab="files">${method ? tr('Method contract', '方法契约') : tr('Files', '文件')}</button></div>
      ${ui.tab === 'files' ? `<div class="eusk-detail-body">${method ? `<div class="eusk-method-summary"><div><span>${component ? tr('Method family', '方法族') : tr('Capability', '能力契约')}</span><code>${esc(component ? skill.methodFamily : skill.capabilityId)}</code></div><div><span>${tr('Execution', '执行方式')}</span><strong>${esc(executionLabel(skill))}</strong></div><div><span>${component ? tr('Method tier', '方法层级') : tr('Claim ceiling', '论断上限')}</span><strong>${esc(component ? tierLabel(skill) : ceilingLabel(skill))}</strong></div></div>
        ...
        <h2>${component ? tr('Reporting bindings', '报告规范') : tr('Required diagnostics', '必要诊断')}</h2>${list(component ? skill.reportingItems : skill.diagnostics)}<h2>${tr('Registered output', '登记产物')}</h2>${list(skill.outputs)}` : user ? `<div class="eusk-file-row">${icon('file', 15)} SKILL.md <small>${skill.size_bytes || 0} B</small></div>
      ...
      : `<div class="eusk-detail-body"><h2>${tr('When to use', '适用场景')}</h2><p>${esc(user || method ? skill.description : skill.scope)}</p>
        ${user ? `<div class="eusk-contract"><div><h3>${tr('Activation stages', '启用阶段')}</h3>${list(skill.stages)}</div><div><h3>${tr('Version', '版本')}</h3><code>sha256:${esc(String(skill.digest || '').slice(0, 16))}</code></div></div>`
          : method ? `<div class="eusk-method-callout"><span>${tr('Task starter', '任务起点')}</span><p>${esc(skill.prompt)}</p></div><div class="eusk-contract"><div><h3>${component ? tr('Method coordinate', '方法坐标') : tr('Inputs to confirm', '能力契约')}</h3><code>${esc(component ? `${skill.methodFamily}.${skill.methodKey}` : skill.capabilityId)}</code></div><div><h3>${tr('Execution', '执行方式')}</h3><p>${esc(executionLabel(skill))}</p></div><div><h3>${tr('Evidence boundary', '证据边界')}</h3><p>${esc(ceilingLabel(skill))}</p></div></div>`
        ...
        <p class="eusk-binding">${masterOff ? tr('The built-in Skill master switch is off in Settings.', '内置技能总开关已在设置中关闭。') + ' ' : ''}${method ? tr('Choosing this method starts a new task with its exact capability and evidence boundary; the plan still requires scientific review before execution.', '选择该方法后会创建新任务，并带入精确的能力契约与证据边界；执行前仍需完成科学计划审阅。') : tr('Changes apply to newly created sessions and runs; existing frozen sessions keep their original skill set.', '启用状态对新建会话和运行生效；已有会话保留原来固化的技能集合。')}</p></div>`}`;
  }
  function installer() {
  ...
  function renderContent() {
    return `<div class="eusk-shell">${rail()}<main class="eusk-main"><div class="eusk-main-inner">
  ...
  async function openSkill(id) {
    ui.selected = id; ui.tab = 'overview'; ui.detail = null; ui.error = ''; rerender();
    const skill = selectedSkill();
  ...
  function bind(root) {
    ...
    hub.addEventListener('click', event => {
      ...
      if (target.dataset.skTab) { ui.tab = target.dataset.skTab; rerender(); return; }
