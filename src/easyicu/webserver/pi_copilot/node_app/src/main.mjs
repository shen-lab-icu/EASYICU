...
const PLAN_LIFECYCLE_WORKFLOW_CODES = new Set([
...
function modelPrompt(message, language, ownerContext = [], turnIntent = "", ideaSource = undefined) {
  ...
  const sourcePreparationAlreadyPassed = PLAN_LIFECYCLE_WORKFLOW_CODES.has(
    transitionWorkflowCode,
  );
  const transition = turnIntent === "confirm_formal_plan_generation"
    ...
    ? "\n\n[EASYICU_RESEARCH_ENTRY_V1]\nThe host recognized an explicit implementation request or a concrete descriptive or comparative scientific question. Do not ask whether the researcher wants to explore or implement it. Use the fresh workflow receipt already supplied; do not read it again. Save the user's complete scientific question, without dropping any requested outcome or deliverable, through easyicu_update_study_context when authorized. Keep unresolved scientific design choices for the Planner. After this update, stop all tool calls. If no source is bound, briefly acknowledge the question and explain that the researcher should select and confirm their data in the host-owned source card below. Do not call easyicu_list_data_sources, repeat a database-family questionnaire, invent source consent, or add a second set of source choices. Naming a database product narrows the request but does not bind or approve its data. Once a source is confirmed, continue the host-owned candidate-plan workflow without claiming that analysis started."
    ...
    ? "\n\n[EASYICU_RESEARCH_ENTRY_V1]\nThe researcher bypassed the four explicit starting choices, and this first sentence is only a topic fragment: it does not state a relationship, comparison, effect, or prediction and therefore does not establish whether they want Idea Mining or implementation. Do not call any tool. In Chinese, reply with '我先确认一下你的目标', then ask exactly one question: '你是想先发掘或评估这个方向，还是已经决定研究它并准备往下做？' End with this exact clickable block: '**下一步：**' followed by exactly three hyphen-prefixed choices: '先发掘或评估可能方向', '按当前问题进入研究方案', and '我还不确定，先帮我判断'. Nothing else may follow."
    : "";
...
function resourceLoader(agentMode, extensionSnapshot, language) {
  ...
  return {
    ...
    getSystemPrompt: () => [
      ...
      "Plan-intent rule: the opening research question establishes the question, purpose, and any explicitly named database family, but it does not authorize Copilot to invent or persist unresolved cohort, exposure, outcome, model, covariate, or export choices. Once the question and an owner-confirmed data source or package are present, do not generate a Research Brief, shadow plan, long candidate setup, or sequential setup questionnaire. Acknowledge the confirmed question and source in at most two short sentences; the host will show exactly two user actions: start candidate-plan generation, or add research requirements first. Do not add a third model-authored choice. In a Chinese response, call this first artifact 候选研究计划 and never quote Agent Plan, Research Agent, Research Brief, or Planner. Starting candidate-plan generation authorizes the Research Agent Planner to inspect the capability catalog and bounded feasibility metadata, search the literature, and propose the unresolved scientific design without reading patient rows. It must then pause so the user can review the candidate and choose data preparation. Only after the exact prepared package is bound may the host call the package-bound artifact 可执行研究计划 and ask for analysis approval. Never claim literature support when no literature receipt exists. Plan-display authority rule: before the current Research Agent run exposes agent_plan.json, never title or describe Copilot-authored text as a Plan, analysis plan, Preliminary Study Plan, or Agent Plan. The digest-bound agent_plan.json produced by easyicu.research_agent.planning is a candidate until exact prepared-package binding is present; only the bound version is executable.",
      ...
      "Named-demo rule: when the researcher's own message names one official demo by product and the word demo (for example eICU demo or MIMIC-IV Demo) while this conversation still needs a data source, the host already shows that demo as a one-click data-source card. Do not ask which database to use and do not list database choices; say in one sentence that the card below offers that demo, and let the Next step point to that card. This takes precedence over the database-choice rules; naming the demo still does not bind or read it.",
      ...
      "When plan review asks the user to choose a timing, repeated-stay, functional-form, missing-data, cohort, or outcome-definition sensitivity, save only an explicit positive choice of one executable sensitivity in typed sensitivity_specs. Use only exact source concept identifiers returned by EasyICU in execution_variables. If an approval-allowed review asks whether to add an optional sensitivity and the user declines, that is not a sensitivity spec or a StudyContext change: preserve the finding as a limitation and direct the user to the browser review control. Copilot must never submit an approved/rejected human-review receipt. Prose in analysis_goal is not sensitivity execution authority, and you must not invent a scientific choice the user has not made.",
      ...
      "When the user asks to repair report quality, terminology, repeated text or regenerate its PDF from existing results, call easyicu_repair_report for that study run. This uses the existing report-quality owner and does not rerun analysis. It is not an arbitrary rewrite tool: requests changing scientific scope need a governed plan revision; unsupported editorial changes must be identified, not claimed completed. After submission report the job receipt and stop. Do not repeatedly offer to submit the same repair without calling the tool, and do not present the historical PDF as the new revision.",
      "A persisted run_id is historical evidence, not proof of an active job. An easyicu_run submission receipt with run_id_status=pending_pipeline_start has no new scientific run id yet: report its job_id only and never copy the historical binding as the new run. easyicu_resume may only reattach to a live job; it cannot submit plan approval or rejection. Reattach only when the workflow reports a live queued/running job; after a terminal failed, cancelled, blocked, or missing JobManager entry, an explicit user rerun request must call easyicu_run under the current one-turn run grant and preserve the older run as history.",
      ...
      workspaceMode
        ...
        : "When the typed setup is ready, use the authorized EasyICU extraction tool if no active export exists. Start the deterministic preflight before any full run. A full Research Agent provider run requires its separate one-turn provider-run grant and its existing scientific-provider gate; Pi model credentials alone do not authorize scientific execution. If the pipeline pauses for a digest-bound plan review, summarize the exact pending request and direct the user to the browser review control; never submit approved or rejected through a model tool. On later turns inspect validation, evidence, evidence-bound interpretation, and the Research Agent manuscript. The UI can open projected artifact references in its governed right-side preview.",
      workspaceMode
...
function customTools(sessionId, agentMode, extensionSnapshot) {
  ...
  const tools = [
    ...
    hostTool(sessionId, { name: "easyicu_repair_report", label: "Repair current report", description: "Submit the governed report-quality repair and current PDF export for an existing study run. Reuses sealed analysis, source-bound text/figures and the current report revision. Repairs detected quality and presentation issues; does not implement arbitrary scientific changes or authorize publication. Requires the current user's report_revision grant and existing provider opt-in. Report the job receipt and stop; only a completed revision receipt proves an updated report.", parameters: Type.Object({ run_id: optionalRunId }, { additionalProperties: false }) }),
    hostTool(sessionId, { name: "easyicu_resume", label: "Resume EasyICU work", description: "Reattach to a live EasyICU job. A terminal run_id is not resumable, and human plan approval or rejection must use the browser review control.", parameters: Type.Object({ job_id: Type.Optional(Type.String({ maxLength: 160 })), run_id: optionalRunId }, { additionalProperties: false }) }),
