...
function customTools(sessionId, agentMode) {
  ...
  return tools.concat([
    ...
    hostTool(sessionId, { name: "easyicu_write_project_file", executionMode: "sequential", label: "Write project file", description: "Create a bounded artifact, or replace one only after reading it and passing its current SHA-256 digest. Requires the reusable host-held workspace-write capability for this message.", parameters: Type.Object({ file: projectFile, content: Type.String({ minLength: 1, maxLength: 262144 }), expected_sha256: Type.Optional(fileSha256) }, { additionalProperties: false }) }),
