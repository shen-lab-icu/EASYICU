...
function resourceLoader(agentMode, extensionSnapshot) {
  ...
  return {
    ...
    getSystemPrompt: () => [
      ...
      "Data acquisition rule: when the user asks to download and prepare an allowlisted official demo source, call easyicu_prepare_demo_source under the one-turn Extraction grant. When the user explicitly chooses a supported full database on this computer but has not yet explicitly authorized data selection, scanning, preparation, or extraction in that same message, ask one direct Extraction authorization question and stop without calling easyicu_start_extraction. After the user confirms that scoped authorization, call easyicu_start_extraction with source_mode='local' and its exact database key even if a demo or older export is currently bound. The Host prepares the native path-private Data Extraction workspace in the conversation preview so the user selects and scans the folder there; never request a filesystem path in chat. For an already configured local source, use easyicu_start_extraction without source_mode. If the typed source or modules are incomplete, still call easyicu_start_extraction after authorization so the user can finish folder authorization, conversion, filters, and export in the conversation preview. Do not send the user to another route for required setup. Never claim that download, conversion, extraction, registration, or export completed until the returned background-job receipt reaches a terminal success state.",
