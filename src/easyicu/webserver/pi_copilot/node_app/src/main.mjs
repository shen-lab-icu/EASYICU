...
function resourceLoader(agentMode, extensionSnapshot) {
  ...
  return {
    ...
    getSystemPrompt: () => [
      ...
      "Data acquisition rule: when the user asks to download and prepare an allowlisted official demo source, call easyicu_prepare_demo_source under the one-turn Extraction grant. For a configured local source, use easyicu_start_extraction. If the user explicitly asks to open, configure, or complete Data Extraction and the typed source or modules are still incomplete, still call easyicu_start_extraction: the Host returns the native path-private Data Extraction workspace in the conversation preview so the user can finish folder authorization, conversion, filters, and export there. Do not claim that no workspace entry exists, and do not send the user to another route for required setup. Never claim that download, conversion, extraction, registration, or export completed until the returned background-job receipt reaches a terminal success state.",
...
function customTools(sessionId, agentMode, extensionSnapshot) {
  ...
  const tools = [
    ...
    hostTool(sessionId, { name: "easyicu_review_cohort", label: "Review cohort and feature distributions", description: "Open a path-free conversational Data Workbench view for cohort size, eligibility/filter funnel, quality, descriptive groups, and up to eight selected feature distributions in one registered export. The browser opens the immutable view; no patient rows or paths are returned to the model.", parameters: Type.Object({ source_id: Type.Optional(Type.String({ minLength: 1, maxLength: 80 })), features: Type.Optional(Type.Array(Type.String({ minLength: 1, maxLength: 160 }), { maxItems: 8 })) }, { additionalProperties: false }) }),
    hostTool(sessionId, { name: "easyicu_open_data_download", label: "Download registered data package", description: "Open a user-controlled browser download for one exact registered EasyICU export. Only the source coordinate and aggregate metadata reach the model; paths and data bytes stay behind the browser click.", parameters: Type.Object({ source_id: Type.Optional(Type.String({ minLength: 1, maxLength: 80 })) }, { additionalProperties: false }) }),
    hostTool(sessionId, { name: "easyicu_preview_icd_cohort", label: "Preview ICD cohort", description: "Resolve a read-only ICU-stay count and aggregate filter funnel for exact ICD include and optional exclude prefixes against one registered source. It reuses the Data Extraction cohort owner and never returns identifiers, raw rows, or host paths.", parameters: Type.Object({ source_id: Type.Optional(Type.String({ minLength: 1, maxLength: 80 })), include_codes: Type.Array(Type.String({ minLength: 1, maxLength: 32 }), { minItems: 1, maxItems: 16 }), exclude_codes: Type.Optional(Type.Array(Type.String({ minLength: 1, maxLength: 32 }), { maxItems: 16 })) }, { additionalProperties: false }) }),
    hostTool(sessionId, { name: "easyicu_review_patient_timeline", label: "Review pseudonymous patient timeline", description: "Open a bounded browser-only time-series view for a pseudonymous entity ordinal in one registered export. Optionally preload up to eight exact EasyICU feature concepts through the existing Patient Review feature owner. Never accepts or returns a direct patient identifier, raw row, timestamp, note, or host path to the model.", parameters: Type.Object({ source_id: Type.Optional(Type.String({ minLength: 1, maxLength: 80 })), entity_ordinal: Type.Optional(Type.Integer({ minimum: 1, maximum: 1000000 })), features: Type.Optional(Type.Array(Type.String({ minLength: 1, maxLength: 80, pattern: "^[a-z][a-z0-9_]*$" }), { maxItems: 8 })) }, { additionalProperties: false }) }),
    ...
    hostTool(sessionId, { name: "easyicu_start_extraction", executionMode: "sequential", label: "Open or start feature extraction", description: "Open the native EasyICU Data Extraction workspace when typed setup is incomplete, or submit the existing extraction owner when it is ready. Requires the one-use Extraction grant; raw paths never come from the model.", parameters: empty }),
