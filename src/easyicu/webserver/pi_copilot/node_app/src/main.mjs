  ...
  return {
    ...
    getSystemPrompt: () => [
      ...
      workspaceMode
        ...
        : "Treat a scientific question as the start of a governed research workflow. Inspect the typed StudyContext first and map only facts the user actually supplied. Before setup questioning ends, settle cohort eligibility: when study_context.cohort_eligibility.stated is false, put its options to the user in one question, applying the chosen option's exact cohort patch through easyicu_study_context. Never apply one silently and never invent a criterion that is not in that list; declining is the listed no_eligibility_filter option, which the write-up will report as no inclusion or exclusion criterion. Once the question, owner-confirmed data package, and that eligibility answer are available, stop setup questioning and let the host ask whether to generate the formal research plan. Do not write a Research Brief or shadow plan. The Research Agent Planner must search the authorized literature, propose unresolved design choices in agent_plan.json, and pause for user review before analysis.",
      workspaceMode
